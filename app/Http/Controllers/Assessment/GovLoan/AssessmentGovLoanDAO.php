...
namespace App\Http\Controllers\Assessment\GovLoan;

use App\Http\Controllers\Helpers\HelpersFunction;
use App\Mail\module\assessment\gov_loan\SendEmailToUserAssessmentGovLoanVehicleStatus;
use App\Mail\module\assessment\gov_loan\SendEmailToUserAssessmentGovLoanVehicleStatusExam;
...
class AssessmentGovLoanDAO
{
    ...
    public function list(Request $request)
    {
        ...
        $TaskFlowAssessmentGovLoan = auth()->user()->vehicle('02', '06');
        $TaskFlowAccessAssessmentGovLoan = auth()->user()->vehicleWorkFlow('02', '06');

        Log::info('AssessmentGovLoanDAO :: list');

        $query = AssessmentGovLoan::orderBy('created_at', 'desc');
        ...
        if($TaskFlowAssessmentGovLoan->fleet_has_limit){
            $query->where('workshop_id', auth()->user()->detail->workshop_id);
        }
        ...
        if($request->search){
            ...
            OR EXISTS (
                SELECT
                    * 
                FROM
                    assessment.assessment_gov_loan_vehicle
                WHERE
                    assessment.assessment_gov_loan_vehicle.assessment_gov_loan_id = assessment.assessment_gov_loan.id 
                    AND upper(assessment.assessment_gov_loan_vehicle.plate_no) LIKE '%".strtoupper($request->search)."%'
                ) 
            )");
        };
    ...
    {
        $id = $request->id ? $request->id : session()->get('govloan_current_detail_id');

        if($request->id){
            $this->detail = AssessmentGovLoan::find($id);
        }
        
        Log::info('data'. $this->detail);
        if($this->detail){
            session()->put('govloan_current_detail_id', $this->detail->id);
        }
    ...
    private function generateRefNumber($data, $workshop_id){
        ...
        $SubModulePrefix = 'P'; //Pinjaman Kerajaan
    ...
    public function upsert(Request $request)
    {
        ...
        Log::info('govloan_current_detail_id => '.session()->get('govloan_current_detail_id'));

        $this->id = session()->get('govloan_current_detail_id');
        $this->detail = AssessmentGovLoan::find($this->id);
        $data = [
        ...
        if($this->detail){
            ...
            $this->url = Route('assessment.gov_loan.register', ['id' => $this->id, 'tab' => '2']);
        } else {
            $data['created_by'] =  Auth::user()->id;
            $data['ref_number'] = $this->generateRefNumber($data,$request->workshop_id);
            $data['app_status_id'] = $this->assessmentStatus("01");
            $query = AssessmentGovLoan::create($data);
            $this->id = $query->id;
    ...
    public function delete(Request $request){
        $ids = $request->ids;
        Log::info($ids);
        $query = AssessmentGovLoan::whereIn('id', $ids);
        $query->update([
    ...
    public function verify(Request $request){
        ...
        $assessment_id = $request->assessment_id ? session()->get('govloan_current_detail_id') : $request->assessment_id;
        $verify = AssessmentGovLoan::find($assessment_id);
        ...
        $vehicle = AssessmentGovLoanVehicle::where('assessment_gov_loan_id', $verify->id)->get();

        foreach($vehicle as $list){
            $saveStatus = AssessmentGovLoanVehicle::find($list->id);
            $data = [
        ...
        // $this->sendEmailNotificationAssessmentGovLoan($verify);
        if($response){
            return [
                    'url' => route('assessment.gov_loan.list')
                ];
        }
    }
    ...
    public function setAppointment(Request $request)
    {
        ...
        $assessment_id = session()->get('govloan_current_detail_id');
        $setAppointment = AssessmentGovLoan::find($assessment_id);
        $data = [
        ...
        if($request->hantar){
            ...
            for ($i = 0; $i < count($request->vehicle_id); $i++) {
                ...
                $save = AssessmentGovLoanVehicle::find($request->vehicle_id[$i]);
        ...
        $response = [
            ...
            'url' => route('assessment.gov_loan.register', [
                'id' => $assessment_id,
                'tab' => 2
            ])
        ];
    ...
    public function examination(){
        ...
        $vehicle = AssessmentGovLoanVehicle::where('assessment_gov_loan_id', $examination->id)
    ...
    public function approval(Request $request){
        for($i = 0; $i < count($request->vehicle_id); $i++){
            $AssessmentGovLoanVehicle = AssessmentGovLoanVehicle::find($request->vehicle_id[$i]);
            if($AssessmentGovLoanVehicle->hasAssessmentVehicleStatus->code == "05" && $request->v_status_code != 0){
                $data = [
                ...
                $AssessmentGovLoanVehicle->update($data);
            }
        }
        // $examination = AssessmentGovLoan::find(session()->get('govloan_current_detail_id'));
        // $data = [
    ...
    public function saveReceipt(Request $request){
        $AssessmentGovLoan = AssessmentGovLoan::findOrFail($request->id);
        $data = [
            'no_receipt' => $request->receipt_no,
        ];

        $AssessmentGovLoan->update($data);
        Log::info($AssessmentGovLoan);
        if($AssessmentGovLoan){
            $response = [
                'code' => 200,
    ...
    private function sendEmailNotificationAssessmentGovLoan($examination){
        $emailBody = [
            'title' => 'Permohonan Penilaian Pinjaman Kerajaan',
            'subject' => 'Permohonan Penilaian Pinjaman Kerajaan '.Carbon::now()->format('d/m/Y'),
        ...
        Mail::to($userEmail)->send(new SendEmailToUserAssessmentGovLoanVehicleStatus($emailBody));
    }

    private function sendEmailNotificationAssessmentNewExamUser($examination){
        $emailBody = [
            'title' => 'Temujanji Penilaian Pinjaman Kerajaan',
            'subject' => 'Temujanji Penilaian Pinjaman Kerajaan '.Carbon::now()->format('d/m/Y'),
            'assessment_id' => $examination->id,
            'no_rujukan' => $examination->ref_number,
            'worksyop' => $examination->hasWorkShop->desc,
            'applicant_name' => $examination->applicant_name,
        ...
        Mail::to($userEmail)->send(new SendEmailToUserAssessmentGovLoanVehicleStatusExam($emailBody));
    }
