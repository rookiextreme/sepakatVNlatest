...
class DaftarKenderaanSeederDepartmentUpdateCawangan extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        foreach ($listMigrate as $key) {
            ...
            $pemilik = $key->pemilik;
            if(strpos($pemilik, "CAWANGAN KEJURUTERAAN ELEKTRIK") !== false){
                $pemilik = "CAWANGAN KEJURUTERAAN ELEKTRIK";
            }

            $pemilik = str_replace('JKR NEGERI', 'JABATAN KERJA RAYA NEGERI', $pemilik);

            $owner_cawangan = RefOwner::select('id','name')->whereRaw("upper(name) like '".$pemilik."%' and (owner_type_id = 3 or owner_type_id = 4)")
            ->first();


            Log::info('Nombor Plate -->'.$key->no_pendaftaran);
            Log::info('Pemilik -->'.$pemilik);
             //Log::info('cawangan_id -->'.$owner_cawangan->id);
