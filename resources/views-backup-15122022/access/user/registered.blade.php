...
<head>
    ...
    <style type="text/css">
        ...
        #role_id ~ .select2-container {
            margin: 0px;
        }

        #role_id ~ .select2-container--classic .select2-selection--single[aria-expanded="false"] {
            height: 35px;
            padding-top: 0px;
            box-shadow: 0px 0px 0px 1px rgb(0 0 0 / 10%), inset 1px 0 5px rgb(0 0 0 / 3%);
            border-radius: 10px;
        }

        #role_id ~ .select2-container--classic .select2-selection--single .select2-selection__arrow {
...
<body class="content">
    ...
    <div class="main-content">
        ...
        <div class="dropdown inline" style="display: inline;">
            <select name="role_id" id="role_id" class="col-md-3">
                @foreach ($roles as $role_index => $role)
                    <option {{$role_id == $role->id ? 'selected' : ''}} value="{{$role->id}}">{{$role->desc_bm}}</option>
                @endforeach
            </select>
    ...
    <script>
        jQuery(document).ready(function() {
            ...
            $('#role_id').select2({
                ...
                role_id = this.value;
                openPgInFrame('{{route('access.user.registered')}}?role_id='+role_id);
            });
