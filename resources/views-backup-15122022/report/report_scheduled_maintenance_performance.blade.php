        ...
        <div class="detailing" style='max-width: fit-content;'>
            ...
            <table style='border:2px solid grey;'>
                ...
                <thead style='border-bottom:2px solid grey;' >
                    {{-- <td class="end"></td> --}}
                    <tr>
                        <td class="small-title p-3">Prestasi Penyenggaraan Berkala</td>
                ...
                <tbody>
                    @foreach($ref_component_lvl1_list as $index_component_lvl1 => $component_lvl1)
                        <tr>
                            ...
                            @for ($i = 1; $i <= 12; $i++)
                                @php
                                    $comp_total = $component_lvl1->hasTotalCheckListLvl1($i);
                                @endphp
                    ...
                    <tr style='border:2px solid grey'>
                        ...
                        @foreach ($ref_month_list as $ref_month)
                            @php
                                $monthly_total = $ref_month->hasTotalCheckListLvl1();
                            @endphp
                    ...
                    <tr style='border-top:2px solid grey'>
                        ...
                        @php
                            $totalAllofMonthInternal = 0;
                        @endphp
                        @foreach ($ref_month_list as $ref_month)
                            @php
                                $comp_totalInternal = $ref_month->hasTotalCheckListLvl1Internal($i);
                            @endphp
                            <td class="col-focus text-count">{{number_format($comp_totalInternal)}}</td>
                            @php
                                $totalAllofMonthInternal = $totalAllofMonthInternal + $comp_totalInternal;
                            @endphp
                        @endforeach

                        <td class="col-focus text-count"  style='border-left:2px solid grey;'>{{number_format($totalAllofMonthInternal)}}</td>
                    </tr>
                    <tr>
                        ...
                        @php
                            $totalAllofMonthExternal = 0;
                        @endphp
                        @foreach ($ref_month_list as $ref_month)
                            @php
                                $comp_totalExternal = $ref_month->hasTotalCheckListLvl1External($i);
                            @endphp
                            <td class="col-focus text-count">{{number_format($comp_totalExternal)}}</td>
                            @php
                                $totalAllofMonthExternal = $totalAllofMonthExternal + $comp_totalExternal;
                            @endphp
                        @endforeach

                        <td class="col-focus text-count"  style='border-left:2px solid grey;'>{{number_format($totalAllofMonthExternal)}}</td>
                    ...
                    <tr style='border-top:2px solid grey'>
                        ...
                        @php
                            $totalAllofMonthlyPerc = 0;
                        @endphp
                        @foreach ($ref_month_list as $ref_month)
                            @php
                                $monthly_total_perc = $ref_month->hasPercentageCheckListLvl1Internal();
                            @endphp
                            <td class="col-focus text-count">{{round($monthly_total_perc)}}%</td>
                            @php
                                $totalAllofMonthlyPerc = $totalAllofMonthlyPerc + $monthly_total_perc;
                            @endphp
                        @endforeach
                        <td class="col-focus big-text-count" style='border-left:2px solid grey'>
                            @php
                                $totalAllofMonth = $totalAllofMonthInternal + $totalAllofMonthExternal;
                            @endphp
                            @if($totalAllofMonthInternal > 0)
                            {{round($totalAllofMonthInternal / $totalAllofMonth * 100)}}%
                            @else 0%
                            @endif
                        </td>
