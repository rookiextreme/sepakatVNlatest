@php
    ...
    $AssessmentAccidentVehicle = AssessmentAccidentVehicle::where('assessment_accident_id',$details['assessment_id'])->first();
@endphp
<!DOCTYPE html>
<html>
...
<body>
    ...
    <div style="max-width:500px;font-size:12px;font-family:Arial, Helvetica, sans-serif">
        ...
        Proses pemeriksaan bagi permohonan No. Rujukan <b>{{$details['no_rujukan']}}</b> telah dihantar.
        Tarikh dan masa temujanji adalah seperti berikut, {{Carbon\Carbon::parse($details['appointment_dt'] )->format('d/m/Y g:i A')}}. Segala pertanyaan berkaitan permohonan boleh diajukan kepada  {{ucwords(strtolower($details['worksyop']))}}</div>
    <br/><br/>

    No. Pendaftaran:
        <b>{{$AssessmentAccidentVehicle->plate_no}}</b>
    <br/><br/><br/>
