...
<form class="row" id="frm_apppointment_vehicle" enctype="multipart/form-data">
    ...
    <div class="row">
            ...
            <div class="row">
                <div class="col-xl-3 col-lg-3 col-md-4 col-sm-5 col-12">
                    <div class="form-group">
                        ...
                        @if($detail && $detail->hasStatus->code == '02' && (auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerAssessment()))
                        ...
                        @else
                            <div class="txt_data ass_new">{{$detail->hasWorkshop->desc}}</div>
                        @endif
                    </div>
                </div>
                <div class="col-xl-2 col-lg-3 col-md-4 col-sm-5 col-12" style="min-width:270px;">
                    <div class="form-group">
                        ...
                        @else
                            <div class="txt_data ass_new caps">
                ...
                @if($detail && $detail->hasStatus->code == '02' && (auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerAssessment()))
                    ...
                    <div class="col-xl-9 col-lg-12 col-md-12 col-sm-12 col-12" style="display: {{ $detail && $detail->is_other_app_dt ? '' : 'none'}}" id="other_datetime_container">
                        ...
                        <div class="row">
                            <div class="col-md-3" >
                                {{-- <div class="input-group date" id="otherAppoinmentDtContainer" data-date="{{$detail && $detail->appointment_dt ? $detail->appointment_dt : ''}}" data-date-format="dd M yyyy">
            ...
            <div class="row">
                ...
                <div class="col-xl-2 col-lg-2 col-md-1 col-sm-2 col-12">
                    <div class="form-group">
                        ...
                        <input type="text" id="ptj_no" name="ptj_no" class="input-sm form-control" onchange="this.value = this.value.toUpperCase()" placeholder="No PTJ" value="{{$detail->no_ptj}}">
                    </div>
                </div>
        ...
        <fieldset>
            ...
            <button type="button" class="btn cux-btn bigger" onclick="checkJob({{$detail->id}})"><i class="fa fa-print"></i> Cetak Borang Pesanan Kerja</button>
            </div>
        </fieldset>
        @endif
    </div>
        ...
        <div class="row">
            <div class="col-md-12 mt-2 mb-2">
                <div class="form-group center">
                    @if($detail->hasStatus->code == '02' && (auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerAssessment()))
...
<script type="text/javascript">
    ...
    function checkJob(assessment_id) {
        ...
        var bpk_no = $('#bpk_no').val();
        var totalprice = $('#grandtotal').text();
        var totalprice2 = $('#grand_total').val();
        let ptj_no = $('#ptj_no').val();

        @if(auth()->user()->isPublic() || auth()->user()->isAdmin())
        console.log(totalprice2);
            if(totalprice2 == 0){
                alert("Borang Pesanan Kerja BPK 1/94 hanya dicetak sekiranya ada bayaran yang perlu dibuat");
            }else{
                if(bpk_no != ''){
                    window.open("{{route('jasperReport')}}?format=pdf&title=Borang Pesanan Kerja&report_name=borang_pesanan_kerja&assessment_new_id="+assessment_id+"&ptj_no="+ptj_no);
                }else{
    ...
    loadAssessmentNewVehicleAppointment = function() {
        $.get("{{ route('assessment.new.vehicle-appointment.list') }}", function(result){
            $('#assessment_new_vehicle_appointment').html(result);
        });
    }
    ...
    function remove(){
        ...
        $.post("{{route('assessment.new.vehicle.cancel')}}", {
            ids: ids,
            '_token': '{{ csrf_token() }}'
        },  function(result){
    ...
    $(document).ready(function(){
        ...
        @if($detail && (auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerAssessment()))
        loadAssessmentNewVehicleAppointment();
        @endif
