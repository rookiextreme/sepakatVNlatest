...
<form class="row" id="frm_vehicle_assessment" enctype="multipart/form-data">
    ...
    <fieldset>
        ...
        <div class="row">
            <div class="col-12" id="assessment_currvalue_vehicle_assessment"></div>
        </div>
    </fieldset>
...
<div class="modal fade" id="assessmentCurrvalueVehicleVerificationModal" data-bs-backdrop="static" data-bs-keyboard="false"
    tabindex="-1" aria-labelledby="assessmentCurrvalueVehicleVerificationModalLabel" aria-hidden="true">
...
<script type="text/javascript">
    ...
    promptSubmitForVerificationModal = function(){
        $('#assessmentCurrvalueVehicleVerificationModal').modal('show');
    }

    submitForVerification = function(){
        $.post("{{ route('assessment.currvalue.verification') }}", {
            '_token': '{{ csrf_token() }}'
        }, function(result){
    ...
    loadAssessmentCurrvalueVehicleAssessment = function() {
        $.get("{{ route('assessment.currvalue.vehicle-assessment.list') }}", function(result){
            $('#assessment_currvalue_vehicle_assessment').html(result);
        });
    }
    ...
    $(document).ready(function(){
        ...
        @if($detail)
        loadAssessmentCurrvalueVehicleAssessment();
        @endif
