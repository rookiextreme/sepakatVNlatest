                                            ...
                                            @foreach ($hasJVEFormDetail->hasJVEForm->hasFormRepairWithSort as $hasFormRepair)
                                                ...
                                                @if(in_array($hasFormRepair->hasRepairMethod->id, explode(',',$hasJVEFormDetail->hasJVEForm->repair_method)))
                                                    ...
                                                    @elseif((auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerMaintenance())  
                                                        && ($hasFormRepair->hasRepairMethod->code == '01' && ($hasJVEFormDetail->hasInternalRepairStatus && in_array($hasJVEFormDetail->hasInternalRepairStatus->code, ['03'])) || $hasFormRepair->hasRepairMethod->code == '01' && in_array($hasJVEFormDetail->hasRepairMethod->code, ['03'])) 
                                                        && $hasJVEFormDetail->hasMaintenanceJobVehicleStatus->code == '05' && $hasFormRepair->hasRepairMethod->code == '02' && (( $hasJVEFormDetail->hasRepairMethod && !in_array($hasJVEFormDetail->hasRepairMethod->code, ['03']))))                                                        
                                                         <button type="button" data-repair-method="{{$hasFormRepair->hasRepairMethod->id}}" onclick="generateFormAssessment({{$vehicle->id}}, '02', this)" class="btn cux-btn small text-uppercase">{{$hasFormRepair->hasRepairMethod->desc}}</button>
