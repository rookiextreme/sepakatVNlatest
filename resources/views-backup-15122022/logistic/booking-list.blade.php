...
<script>
    ...
    function searching(value, trigger){
        ...
        if((searching && trigger == 'click')){
            window.location.href = "{{route('logistic.booking.list', ['status_code' => $status_code])}}&search="+searching;
        } else {
            if(keycode ==  13 || !value){
                window.location.href = "{{route('logistic.booking.list', ['status_code' => $status_code])}}&search="+value;
            }
        }
    }
