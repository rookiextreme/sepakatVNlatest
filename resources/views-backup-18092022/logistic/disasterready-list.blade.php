    ...
    <div class="sect-top">
        ...
        <div class="dropdown inline" style="display: inline;">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                ...
                <li><a class="dropdown-item {{$status_code == 'all_inprogress' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('logistic.disasterready.list', ['status_code' =>  'all_inprogress'])}}')">Dalam Proses 
                    @if($totalProcess>0)
                ...
                @if($totalDraft > 0)
                    <li><a class="dropdown-item {{$status_code == '01' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('logistic.disasterready.list', ['status_code' =>  '01'])}}')">Draf 
                        <span class="badge bg-secondary">{{$totalDraft}}</span>
                    </a></li>
                @endif
                <li>
                    <a class="dropdown-item {{$status_code == '02' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('logistic.disasterready.list', ['status_code' =>  '02'])}}')">Semakan 
                    @if($totalVerification > 0)
                        <span class="badge bg-secondary">{{$totalVerification}}</span>
                    @endif
                </a></li>
                <li>
                    <a class="dropdown-item {{$status_code == '03' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('logistic.disasterready.list', ['status_code' =>  '03'])}}')">Kelulusan 
                    @if($totalApproval > 0)
                ...
                <li><a class="dropdown-item {{$status_code == '06' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('logistic.disasterready.list', ['status_code' =>  '06'])}}')">Selesai 
                    @if($totalCompleted>0)
        ...
        <div class="btn-group">
            ...
            <button class="btn cux-btn bigger" xaction="delete_all" data-bs-toggle="modal" data-bs-target="#deleteDisasterReadyModal" disabled type="button" data-toggle="popover" data-trigger="focus" data-placement="top" data-content="Some info"><i class="fal fa-trash-alt"></i> Hapus</button>
            @endif
            @if ($TaskFlowAccessDisasterReady->mod_fleet_verify)
            <button class="btn cux-btn bigger" xaction="verify_all" data-bs-toggle="modal" data-bs-target="#verifyDisasterReadyModal" disabled type="button" data-toggle="popover" data-trigger="focus" data-placement="top" data-content="Semak"><i class="fa fa-check"></i>&nbsp;Semak</button>
            @endif

            @if ($TaskFlowAccessDisasterReady->mod_fleet_approval)
            <button class="btn cux-btn bigger" xaction="approve_all" data-bs-toggle="modal" data-bs-target="#approvalDisasterReadyModal" disabled type="button" data-toggle="popover" data-trigger="focus" data-placement="top" data-content="Sah"><i class="fa fa-check"></i>&nbsp;Sah</button>
            @endif
        </div>
        <div class="dropdown inline float-end">
            <div class="input-group">
                ...
                @endif
                <input class="form-control search" size="16" type="search" onfocus="(this.type='date')" onfocusout="(this.type='text')"
                    placeholder="Tarikh Tempahan" id="searching" value="{{$search}}">
                ...
                @foreach ($booking_list as $booking)
                    <tr>
                        ...
                        <td class="col-edit">
                            ...
                            <button type="button" class="btn" onclick="parent.openPgInFrame('{{route('logistic.disasterready.register', [ 'id' => $booking->id])}}')" data-bs-toggle="tooltip" data-bs-placement="top" ><i class="fa fa-pencil-alt"></i></button>
                        </td>
                        <td>{{$booking->booking_person_name}}<br/><small>{{$booking->hasAgency ? $booking->hasAgency->desc : ''}}</small></td>
                        ...
                        <td class="text-center">
                            ...
                            <a href="{{route('logistic.disasterready.slip', [ 'booking_id' => $booking->id ])}}">Lihat Slip</a>
                            @endif
                        </td>
                    </tr>
                @endforeach
...
<script>
    ...
    function searching(value, trigger){
        ...
        if((searching && trigger == 'click')){
            window.location.href = "{{route('logistic.disasterready.list', ['status_code' => $status_code])}}&search="+searching;
        } else {
            if(keycode ==  13 || !value){
                window.location.href = "{{route('logistic.disasterready.list', ['status_code' => $status_code])}}&search="+value;
            }
        }
    }
    ...
    jQuery(document).ready(function() {
        ...
        $('#clear_search').on('click', function(){
            window.location.href = "{{route('logistic.disasterready.list', ['status_code' => $status_code])}}&search=";
        });
