    ...
    <table class="table table-borderless" id="letter{{$template_type_code}}" style="width: 100%">
                ...
                <table class="float-end" style="width: 400px;">
                    ...
                    <tr>
                        <td>Tarikh</td><td>:</td><td>
                            ...
                            @else
                                <input onchange="saveElement(this)"  type="text" name="date" id="date" value="{{$detail->date ?: \Carbon\Carbon::now()->format('d F Y')}}">
                            @endif
                        </td>
                    </tr>
                </table>
                ...
                <table class="float-start" style="width:{{$is_preview ? '400px': '100%';}}">
                    <tr>
                        <td>
                            ...
                            <textarea maxlength="200" onchange="saveElement(this)" style="width: 400px; resize: none;" name="address_to" id="" cols="30" rows="7" class="form-control" placeholder="Butiran Penerima Surat">{{$detail->address_to}}</textarea>
                            @endif
                        </td>
                    </tr>
                </table>
        ...
        <tr>
            <td colspan="3" class="form-label fs-5" style="line-height: 21px">
                PEMERIKSAAN DAN PENILAIAN KEROSAKAN KENDERAAN
                @if($is_preview)
                    {{$detail->vip_address}}
                @else
                    <input onchange="saveElement(this)" style= "width:200px" type="text" name="vip_address" id="vip_address" value="{{$detail->vip_address}}">
                @endif
                JENIS {{$vehicleBrandName}} {{$vehicleModelName}} NO. PENDAFTARAN : {{$vehiclePlateNo}}
        ...
        <tr>
            <td colspan="3" class="" style="line-height: 21px">
                ...
                dan bersetuju dengan kerja-kerja yang dicadang oleh pihak
                @if($is_preview)
                    {{$detail->body_detail}}
                @else
                    <input onchange="saveElement(this)" style= "width:200px" type="text" name="body_detail" id="body_detail" value="{{$detail->body_detail}}">
                @endif
                .
            </td>
        </tr>
        ...
        <tr>
            <td colspan="3" class="" style="line-height: 21px">
                3. Sekiranya ada penambahan kerja penyenggaraan, perlu merujuk kepada pihak kami terlebih dahulu bagi tujuan pengesahan kerosakan.
            </td>
        </tr>
        ...
        <tr>
            <td colspan="3">
                ...
                <textarea onchange="saveElement(this)" style="width: 300px; resize: none;" name="signature_quote" id="" cols="30" rows="3" class="form-control" placeholder="PRIHATIN RAKYAT: DARURAT MEMERANGI COVID-19">{{$detail->signature_quote}}</textarea>
                @endif
                ...
                <br/>
                    ...
                    <textarea onchange="saveElement(this)" style="width: 300px; resize: none;" name="officer_designation" id="" cols="30" rows="4" class="form-control" placeholder="Ketua Jurutera Mekanikal&#10JKR Woksyop Persekutuan">{{$detail->officer_designation}}</textarea>
                    @endif<br/>
            </td>
        </tr>
