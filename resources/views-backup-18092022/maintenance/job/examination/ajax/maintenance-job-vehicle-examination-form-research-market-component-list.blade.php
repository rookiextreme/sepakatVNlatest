...
<div class="table-responsive">
            ...
            @endphp
                <tr>
                    ...
                    <div style="height: 1px; width:1px; overflow: hidden;" name="" id="copy_this_{{$component->id}}">{{$indexItemCopy}}. {{$component && $component->detail ? $component->detail : ''}} <br/><br/>
                        @if($component && $component->hasManyResearchMarketComponent->count()>0)
                            @foreach ($component->hasManyResearchMarketComponent as $componentSub)
                                @switch($componentSub && $componentSub->lvl) 
                                    @case(2){{$indexItemCopy}}.{{$loop->index+1}}. {{$componentSub->hasCompLvl2 && $componentSub->hasCompLvl2->component}} <br/>
                                        @break 
                                    @case(3){{$indexItemCopy}}.{{$loop->index+1}}. {{$componentSub->hasCompLvl3 && $componentSub->hasCompLvl3->component}} <br/>
                                        @break 
                                    @default 
                                @endswitch 
                            @endforeach 
                    ...
                    <td class="text-uppercase">
                        ...
                        {{$component->detail}} <br>
                        SISTEM DIPILIH :{{$component->hasRefComponentLvl1 ? $component->hasRefComponentLvl1->component:''}}
                    ...
                    <td class="text-center">
                        ...
                        @if($can_edit)
                            <input class="form-control" type="hidden" onchange="selfRecalculate(this, {{$component->id}})" name="quantity" id="quantity_component_id_{{$component->id}}" value="{{$component->quantity}}">
                        @endif
                    </td>
                    <td style="text-align: right;">
                        @if($can_edit)
                        <input class="form-control" type="text" data-id="{{$component->id}}" onkeypress="return isNumber(this)" onchange="selfRecalculate(this, {{$component->id}})" name="per_unit" id="per_unit_component_id_{{$component->id}}" value="{{$component->per_price}}">
                        @else
    ...
    <div class="float-end mb-2 mt-1">
        ...
        <label class="ms-2 form-label d-inline fs-6" id="grandTotal">{{$detail ? number_format($detail->price_budget, 2) : null}}</label>
    </div>
</div>
