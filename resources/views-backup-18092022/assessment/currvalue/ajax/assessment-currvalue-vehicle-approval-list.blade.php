                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            type="button"
                            class="btn cux-btn small {{$vehicle->hasAssessmentVehicleStatus->code == "05" ? 'btn-check-custom' : ''}}"
                                {{$vehicle->hasAssessmentVehicleStatus->code == "05" ? '' : 'disabled'}}
                                onclick="generateFormExaminationcurrvalue({{$vehicle->id}}, '04')"> {{$vehicle->app_datetime ? 'Tukar': 'Semak'}}</button>
                        </div>
                    </td>
...
{{$vehicleList->links('pagination.ajax-default', [
   'targetDivList' =>  '#assessment_currvalue_vehicle_approval',
   'params' => $params
])}}
...
<script type="text/javascript">
    ...
    generateFormExaminationcurrvalue = function(vehicle_id, assessment_type_code_selected){
        if(vehicle_id){
