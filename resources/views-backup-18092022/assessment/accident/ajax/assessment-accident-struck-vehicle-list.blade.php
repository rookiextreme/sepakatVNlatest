            ...
            @foreach ($vehicleList as $index => $vehicle)
                <tr class="vehicle">
                    @if($hasAssessmentAccidentDetail->hasStatus->code == '01')
                        ...
                        <td class="lcal-btn" data-type="struck" onclick="editAssessmentVehicle({{$vehicle->id}}, this)"><i class="fa fa-pencil-alt"></i></td>
                    @endif
                    <td>{{$vehicleList->firstItem() + $index}}</td>
                    <td class="special"><a data-type="struck" href="javascript:editAssessmentVehicle({{$vehicle->id}}, this)">{{$vehicle->accwit_regno}}</a></td>
