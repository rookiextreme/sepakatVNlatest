...
@endphp
<form id="frm_application" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="section" value="detail">
    <div class="messages"></div>
    <input type="hidden" name="representing" id="representing" value="{{$detail ? $detail->representing : 'police'}}"><!--police or driver-->
    @if(!$detail ||$is_myApp && in_array($detail->hasStatus->code, ['01']) || auth()->user()->isAdmin())
    <div class="row">
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <label for="" class="form-label">Anda mewakili : <span class="text-danger">*</span></label>
            <div class="radio-highlight">
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="as-who" id="representing1" value="police" {{$detail && $detail->representing == 'police' ? 'checked' : 'checked'}}>
                    <label class="form-check-label" for="representing1">Pegawai Polis</label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="as-who" id="representing2" value="driver" {{$detail && $detail->representing == 'driver' ? 'checked' : ''}}>
                    <label class="form-check-label" for="representing2">Pemandu Kenderaan Kerajaan</label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="as-who" id="representing3" value="office">
                    <label class="form-check-label" for="representing3">Wakil Jabatan <i class="fa fa-info-circle" id="jabatan-tip" data-bs-toggle="tooltip" data-bs-placement="top" title="Wakil jabatan yang sama dengan pemandu yang terlibat"></i></label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="as-who" id="representing4" value="public" {{$detail && $detail->representing == 'public' ? 'checked' : ''}}>
                    <label class="form-check-label" for="representing4">Orang Awam</label>
                </div>
            </div>
        </div>
    </div>
    ...
    <fieldset>
        ...
        <div class="row">
            <div class="col-xl-3 col-lg-4 col-md-4 col-sm-5">
                <label for="" class="form-label text-dark" id="pemohon-is">Nama Pemohon <span class="text-danger">*</span></label>

                @if(!$detail ||$is_myApp && in_array($detail->hasStatus->code, ['01']) || auth()->user()->isAdmin())
                    <input type="text" name="applicant_name" id="applicant_name" class="form-control" autocomplete="off"
                    onchange="this.value = this.value.toUpperCase()" value="{{$detail ? $detail->applicant_name : strtoupper(auth()->user()->name)}}" required>
                @else
                    <div class="txt-data ass_acc">{{$detail ? $detail->applicant_name : strtoupper(auth()->user()->name)}}</div>
                @endif
            </div>
            <div class="col-xl-2 col-lg-2 col-md-2 col-sm-3">
                ...
                @if(!$detail ||$is_myApp && in_array($detail->hasStatus->code, ['01']) || auth()->user()->isAdmin())
                    <input type="tel" name="phone_no" id="phone_no" class="form-control" autocomplete="off"
                    value="{{$detail ? $detail->phone_no : auth()->user()->detail->telbimbit}}"
                    onkeyup="this.value = !isNaN(this.value) ? this.value : ''; " maxlength="15" required>
                @else
                <div class="txt-data ass_acc">{{$detail ? $detail->phone_no : auth()->user()->detail->telbimbit}}</div>
                @endif
            </div>
            <div class="col-xl-2 col-lg-4 col-md-4 col-sm-4 col-sm-4">
                ...
                @if(!$detail ||$is_myApp && in_array($detail->hasStatus->code, ['01']) || auth()->user()->isAdmin())
                    ...
                    value="{{$detail ? $detail->email : auth()->user()->email}}" maxlength="40" required>
                @else
                    <a href="mailto:{{$detail ? $detail->email : auth()->user()->email}}" target="blank" class="txt-data-small ass_acc" style="top:-50px;line-height:0px">{{$detail ? $detail->email : auth()->user()->email}}</a>
                @endif
            </div>
        </div>
        <div id="jabatan" style="display:{{$detail && $detail->representing == 'driver' ? 'block': 'none'}};">
            <div class="row">
                        ...
                        <div class="input-group">
                            <select class="form-control form-select" id="agency_id" name="agency_id" >
                                ...
                                @foreach ($agency_list as $agency)
                                    <option
                                    {{($detail && $detail->hasAgency && $detail->hasAgency->id == $agency->id) || (auth()->user()->detail->hasAgency && auth()->user()->detail->hasAgency->id == $agency->id) ? 'selected'  : ''}}
                                     value="{{$agency->id}}">{{strtoupper($agency->desc)}}</option>
                                @endforeach
                            </select>
                        </div>
                    @else
                        <div class="txt-data ass_acc">{{$detail && $detail->hasAgency ? $detail->hasAgency->desc : null}}</div>
                    @endif
                </div>
                <div class="col-xl-3 col-lg-4 col-md-5 col-sm-6">
                    ...
                    @else
                        <div class="txt-data ass_acc">{{$detail ? $detail->department_name : null}}</div>
                    @endif
                </div>
            </div>
            <div class="row">
                <div class="col-xl-3 col-lg-3 col-md-4 col-sm-6">
                    ...
                    @if(!$detail ||$is_myApp && in_array($detail->hasStatus->code, ['01']) || auth()->user()->isAdmin())
                        <div class="input-group">
                            <textarea class="form-control" name="address" id="address" maxlength="250" onchange="this.value = this.value.toUpperCase()" cols="30" rows="3">{{$detail ? $detail->address : auth()->user()->detail->address}}</textarea>
                        </div>
                    @else
                        <div class="txt-data ass_acc">{{$detail ? $detail->address : null}}</div>
                    @endif
                </div>
                <div class="col-xl-6 col-lg-6 col-md-7 col-sm-6">
                    <div class="row">
                        <div class="col-xl-2 col-lg-3 col-md-3 col-sm-6">
                            ...
                            @else
                                <div class="txt-data ass_acc">{{$detail ? $detail->postcode : null}}</div>
                            @endif
                        </div>
        ...
        <div class="row">
            <div class="col-xl-3 col-lg-4 col-md-5 col-sm-6">
                ...
                <label for="" class="form-label text-dark">No Rujukan <span class=""></span></label>
                    <div class="txt-data ass_acc thin-underline" style="height:45px">
                ...
                @if(!$detail ||$is_myApp && in_array($detail->hasStatus->code, ['01']) || auth()->user()->isAdmin())
                    <select id="workshop_id" name="workshop_id" class="form-control form-select" required>
                        <option></option>
                ...
                @else
                    <div class="txt-data ass_acc">{{$detail && $detail->hasWorkShop ? $detail->hasWorkShop->desc : null}}</div>
                @endif
            </div>
            <div class="col-xl-3 col-lg-4 col-md-5 col-sm-6">
                ...
                <div class="row">
                    ...
                    @else
                        <div class="txt-data ass_acc">{{$detail && $detail->appointment_dt1 ? Carbon\Carbon::parse($detail->appointment_dt1 )->format('d M Y G:i A') : ''}}</div>
                    @endif
                </div>
                <div class="row">
                    <div class="col-12">
    ...
    <div class="form-group center">
        @if($detail)
            ...
            {{-- <button class="btn btn-module verify {{$detail && $detail->id && $detail->hasVehicle ? : dim}}" data-id="{{$detail ? $detail->id : null}}">Hantar</button> --}}
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        @endif
            ...
            $('input:radio[name="as-who"]').change(function(){
            if (this.checked && this.value == 'police') {
                $('#pemohon-is').html('Nama Pegawai Polis <span class="text-danger">*</span>');
                $('#jabatan').slideUp('fast');
                $('#representing').val('police');

                //check if driver_name has been filled up
                var driver_name = $('#driver_name').val();
                var applicant_name = $('#applicant_name').val();
                if(driver_name == applicant_name){
                    $('#driver_name').val('');
                }
            }else if (this.checked && this.value == 'driver') {
                $('#pemohon-is').html('Nama Pemandu <span class="text-danger">*</span>');
                $('#jabatan').slideDown('fast');
                $('#representing').val('driver');

                //check if driver_name has been filled up
                var driver_name = $('#driver_name').val();
                var applicant_name = $('#applicant_name').val();
                if(driver_name == ''){
                    $('#driver_name').val(applicant_name);
                }
            }else if (this.checked && this.value == 'office') {
                $('#pemohon-is').html('Nama Wakil <span class="text-danger">*</span>');
                $('#jabatan').slideDown('fast');
                $('#representing').val('office');
