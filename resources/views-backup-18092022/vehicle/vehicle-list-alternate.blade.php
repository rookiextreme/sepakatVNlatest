...
<div class="sect-top">
    ...
    <div class="dropdown inline" style="display: inline;">
        ...
        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
            ...
            <li><a class="dropdown-item @if($ownership == 'jkr' && $status == null) active @endif" onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['status' => '', 'xmode' => $xmode, 'ownership' => 'jkr', 'fleet_view' =>  'department', 'limit' => $limit, 'offset' => $offset ])}}')">Milik JKR</a></li>
            ...
            <li><a class="dropdown-item @if($status == 'draf') active @endif" onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['status' => 'draf', 'xmode' => $xmode, 'limit' => $limit, 'offset' => $offset ])}}')" >Draf <span class="badge bg-secondary"><?=$totaldraf?></span></a></li>
            @if ($TaskFlowAccessVehicle->mod_fleet_verify || auth()->user()->isAdmin() || auth()->user()->isPublic())
            <li><a class="dropdown-item @if($status == 'verification') active @endif" onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['status' => 'verification', 'xmode' => $xmode, 'limit' => $limit, 'offset' => $offset ])}}')" >Menunggu Semakan <span class="badge bg-secondary"><?=$totalverification?></span></a></li>
            @endif
            @if ($TaskFlowAccessVehicle->mod_fleet_approval || auth()->user()->isAdmin() || auth()->user()->isPublic())
            <li><a class="dropdown-item @if($status == 'approval') active @endif" onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['status' => 'approval', 'xmode' => $xmode, 'limit' => $limit, 'offset' => $offset ])}}')" >Menunggu Pengesahan <span class="badge bg-secondary"><?=$totalapproval?></span></a></li>
            @endif
        </ul>
...
<div class="main-content">
    @if($xmode == 'list')
        <div class='row' style='margin-top:10px;margin-bottom:10px;display:none'>
            <div class='col-3'>
                <div class='btn-group'>
                    <button class='btn cux-btn nav-btn' onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['offset' => ($offset-$limit), 'ownership' => Request('ownership'), 'fleet_view' => $fleet_view, 'owner_id' => $owner_id, 'state_id' => $state_id, 'type_id' => $type_id ])}}')" {{$offset == 0 ? "disabled":""}} ><i class="fa fa-arrow-left"></i>222</button>
                    <button class='btn cux-btn nav-btn' onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['offset' => ($offset+$limit), 'ownership' => Request('ownership'), 'fleet_view' => $fleet_view, 'owner_id' => $owner_id, 'state_id' => $state_id, 'type_id' => $type_id ])}}')" {{$offset+$limit > $total_fleet ? "disabled":""}}><i class="fa fa-arrow-right"></i>222</button>
                </div>
        ...
        @if($total_fleet > 0)
            <div style='margin-top:10px;margin-bottom:10px'>
                <div class='btn-group'>
                    <button class='btn cux-btn nav-btn' onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['offset' => ($offset-$limit), 'limit' => $limit,  'fleet_view' => $fleet_view, 'search' => $search, 'owner_id' => $owner_id, 'state_id' => $state_id, 'type_id' => $type_id, 'ownership' => $ownership ])}}')" {{$offset == 0 ? "disabled":""}} ><i class="fa fa-arrow-left"></i></button>
                    <button class='btn cux-btn nav-btn' onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['offset' => ($offset+$limit), 'limit' => $limit, 'fleet_view' => $fleet_view, 'search' => $search, 'owner_id' => $owner_id, 'state_id' => $state_id, 'type_id' => $type_id, 'ownership' => $ownership ])}}')" {{$offset+$limit > $total_fleet ? "disabled":""}}><i class="fa fa-arrow-right"></i></button>
                </div>
    ...
    @elseif($xmode == 'catelog')
        <div xmode="catelog" class="catalogue">
            ...
            @if($total_fleet > 0)
                <div style='margin-top:10px;margin-bottom:10px'>
                    <div class='btn-group'>
                        <button class='btn cux-btn nav-btn' onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['offset' => ($offset-$limit), 'limit' => $limit,  'fleet_view' => $fleet_view, 'search' => $search, 'owner_id' => $owner_id, 'state_id' => $state_id, 'type_id' => $type_id, 'xmode' => $xmode, 'ownership' => $ownership ])}}')" {{$offset == 0 ? "disabled":""}} ><i class="fa fa-arrow-left"></i></button>
                        <button class='btn cux-btn nav-btn' onclick="parent.openPgInFrame('{{route('vehicle.list-alternate', ['offset' => ($offset+$limit), 'limit' => $limit, 'fleet_view' => $fleet_view, 'search' => $search, 'owner_id' => $owner_id, 'state_id' => $state_id, 'type_id' => $type_id, 'xmode' => $xmode, 'ownership' => $ownership ])}}')" {{$offset+$limit > $total_fleet ? "disabled":""}}><i class="fa fa-arrow-right"></i></button>
                    </div>
                    <span class='nav-text'>&nbsp;&nbsp;&nbsp;{{($offset+1)}} - {{$total_fleet<($offset+$limit) ? $total_fleet : ($offset+$limit) }} of {{$total_fleet}}</span>
                </div>
                <p>&nbsp</p>
...
<script>
    ...
    function downloadExcel(){
        ...
        $.ajax({
            ...
            success: function(result, status, xhr){
                ...
                var filename = (matches != null && matches[1] ? matches[1] : 'Senarai Kenderaan '+fleet_view+'.xlsx');
    ...
    $(document).ready(function() {
        ...
        $('#owner_id').select2({
            ...
            state_id = '';
            type_id = '';
            let url = '{{route('vehicle.list-alternate')}}?owner_id='+this.value+'&state_id='+state_id+'&type_id='+type_id+'&xmode='+xmode+'&limit='+limit;
        ...
        $('#state_id').select2({
            ...
            type_id = '';
            let url = '{{route('vehicle.list-alternate')}}?owner_id='+owner_id+'&state_id='+this.value+'&type_id='+type_id+'&xmode='+xmode+'&limit='+limit;
            parent.openPgInFrame(url);
        });
        ...
        $('#type_id').select2({
            ...
            let url = '{{route('vehicle.list-alternate')}}?owner_id='+owner_id+'&state_id='+state_id+'&type_id='+this.value+'&xmode='+xmode+'&limit='+limit;
            parent.openPgInFrame(url);
        });
