...
background: linear-gradient(0deg, rgba(203,203,203,1) 0%, rgba(227,231,225,1) 100%);
    ...
    .job-box table tr:hover {
        background-color: transparent;
    }

    .job-box table td:hover {
        background-color: transparent;
    }
...
<body>
    ...
    @if(auth()->user()->isForemenAssessment() && auth()->user()->isForemenMaintenance())
        <div class="container mt-4 ps-0">
            <div class="btn-group">
                <button type="button" onclick="parent.openPgInFrame('{{route('access.operation.dashboard', ['dashboard_type' => 'assessment'])}}')" class="btn cux-btn bigger {{$dashboard_type == 'assessment' ? 'active' : ''}}">Dashboard Penilaian</button>
                <button type="button" onclick="parent.openPgInFrame('{{route('access.operation.dashboard', ['dashboard_type' => 'maintenance'])}}')" class="btn cux-btn bigger {{$dashboard_type == 'maintenance' ? 'active' : ''}}">Dashboard Penyenggaraan</button>
            </div>
        </div>
    @endif
    ...
    @if(auth()->user()->isForemenAssessment() && auth()->user()->isForemenMaintenance())
        @switch($dashboard_type)
            @case('assessment')
                @include('access_ops.foremen-assessment-dashboard')
                @break
            @case('maintenance')
                @include('access_ops.foremen-maintenance-dashboard')
                @break
        @endswitch
