...
//    $booking_vehicle_grant_list = $DisasterReadyVehicleDAO->booking_vehicle_grant_list;
...
<body class="content">
<div class="mytitle">Senarai Kenderaan <span>Konsesi</span></div>
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="javascript:openPgInFrame('{{route('logistic.overview')}}');"><i class="fal fa-home"></i></a></li>
      <li class="breadcrumb-item"><a href="#">Logistik</a></li>
      <li class="breadcrumb-item active" aria-current="page">Rekod Kenderaan Konsesi</li>
    </ol>
</nav>
<input type="hidden" name="filter-opt" id="filter-opt" value="{{$filterOpt}}">
<div class="main-content">
    <div class="btn-group">
        <button type="button" class="btn cux-btn small" onclick="parent.openPgInFrame('{{route('vehicle.register', ['fleet_view' => 'grant', 'prev_url' => route('logistic.booking.vehicle.grant.list')])}}')"><i class="fa fa-plus"></i> Kenderaan Konsesi</button>
    ...
    <div class="row">
        <div class="table-responsive pt-0">
            <table id="fleet-ls" class="table-custom mt-0 no-footer stripe" style="height:{{count($booking_vehicle_grant_list) == 1 ? '200px;' : ''}}100%;">
                <thead>
                    <tr>
                        <th>Bil</th>
                ...
                <tbody id="sortable">
                    @foreach ($booking_vehicle_grant_list as $index => $booking_vehicle_grant)
                        <tr>
                            <td>{{($index + 1) + $offset}}</td>
                            <td>{{$booking_vehicle_grant->no_pendaftaran}}</td>
                            <td>{{$booking_vehicle_grant->hasFleetPlacement->desc}}</td>
                        </tr>
                    @endforeach
                    @if (count($booking_vehicle_grant_list) == 0)
                    <tr class="no-record">
    ...
    <div class="modal-dialog" style="margin-top:22vh;-webkit-border-radius: 12px;-moz-border-radius: 12px;border-radius: 12px;">
        <div class="modal-content awas" style="background-image: url({{asset('my-assets/img/awas.png')}});">
            ...
            <div class="modal-footer float-start">
                <span class="btn btn-module" xaction="close" onclick="parent.openPgInFrame('{{route('logistic.booking.vehicle.grant.list')}}')" style="display: none" data-bs-dismiss="modal">Tutup</span>
...
{{$booking_vehicle_grant_list->links('pagination.default', [ 'limit' => $limit])}}
...
<script>
    ...
    function searching(value, trigger){
        ...
        if((searching && trigger == 'click')){
            window.location.href = "{{route('logistic.booking.vehicle.grant.list')}}?search="+searching+"&filterOpt="+filterOpt;
        } else {
            if(keycode ==  13 || !value){
                window.location.href = "{{route('logistic.booking.vehicle.grant.list')}}?search="+value+"&filterOpt="+filterOpt;
            }
        }
    }
