@php
   ...
   $LogisticTaskDriverDAO->mount();
   $task_list = $LogisticTaskDriverDAO->task_list;
@endphp
<!DOCTYPE html>
    ...
    <tbody id="sortable">
        @foreach ($task_list as $task)
            <tr>
                ...
                <td>{{$task->reason}}</td>
                <td class="text-center">{{\Carbon\Carbon::parse($task->created_at)->format('d/m/Y')}}</td>
                {{-- <td class="text-center">{{\Carbon\Carbon::parse($task->start_datetime)->format('d/m/Y h:m:s A')}}</td>
                <td class="text-center">{{\Carbon\Carbon::parse($task->end_datetime)->format('d/m/Y h:m:s A')}}</td> --}}
                <td class="text-center">{{$task->status_name}}</td>
                <td class="text-center">
                    @if($task->status_code == '06')
                    <a href="{{route('logistic.driver-task.detail', [ 'booking_id' => $task->booking_id , 'category' => $task->category, 'fingger_print_id' => '' ])}}">Kemaskini Tugasan</a>
                    @endif
                    @if($task->status_code == '07')
                    <a href="{{route('logistic.driver-task.detail', [ 'booking_id' => $task->booking_id, 'category' => $task->category, 'fingger_print_id' => '' ])}}">Lihat Tugasan</a>
                    @endif
                </td>
            </tr>
        @endforeach
