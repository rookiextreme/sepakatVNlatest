...
<form class="row" id="frm_apppointment_vehicle" enctype="multipart/form-data">
    ...
    <fieldset>
        ...
        <div class="row">
            <div class="col-xl-3 col-lg-3 col-7">
                <div class="form-group">
                    ...
                    @else
                        <div class="txt_data ass_dis">{{$detail->hasWorkshop ? $detail->hasWorkshop->desc : ''}}</div>
                    @endif
                </div>
            </div>
            <div class="col-xl-3 col-lg-3 col-md-4 col-5">
                <div class="form-group">
                    ...
                    @else
                    <div class="txt_data ass_dis">{{$detail->appointment_dt ? \Carbon\Carbon::parse($detail->appointment_dt)->format('d/m/Y') : 'MENUNGGU TEMUJANJI'}} <small class="ass_dis"> {{$detail->appointment_dt ? \Carbon\Carbon::parse($detail->appointment_dt)->format('g:i A') : ''}}</small></div>
                    @endif
                </div>
            </div>
            @if($detail && $detail->hasStatus->code == '02' && auth()->user()->isAssistEngineerAssessment())
        ...
        <div class="row" id="assessment_disposal_vehicle_appointment">
    ...
    <fieldset>
        <legend>MAKLUMAT KENDERAAN</legend>
            <div class="col-12 pt-2 mb-2">
                <div class="btn-group">
                    @if(
                        ($detail->hasStatus->code == '01' && $detail->created_by == Auth::user()->id) ||
                        ((auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerAssessment()))
                        )
        ...
        @if(Auth::user()->isAdmin() || 
            (auth()->user()->isEngineer() || auth()->user()->isEngineerAssessment()) || 
            (auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerAssessment())
            )
            <div class="col-xl-12 col-lg-12 col-md-12" id="assessment_disposal_vehicle_appointment">
...
<script type="text/javascript">
    ...
    loadAssessmentDisposalVehicleAppointment = function() {
        $.get("{{ route('assessment.disposal.vehicle-appointment.list') }}", function(result){
            $('#assessment_disposal_vehicle_appointment').html(result);
        });
    }

    updateAssessmentNewVehicleAppointmentDisposal =  function(data){
        $.ajax({
            url: "{{ route('assessment.disposal.register.save') }}",
            type: 'post',
            data: data,
            ...
            success: function(response) {
                ...
                // loadAssessmentDisposalVehicleAppointment();
    ...
    $(document).ready(function(){
        ...
        @if($detail && 
            Auth::user()->isAdmin() || 
            Auth::user()->isEngineerAssessment() ||
            auth()->user()->isAssistEngineerAssessment()
        )
        loadAssessmentDisposalVehicleAppointment();
        @endif
        ...
        let app_dt_1_container = $('#otherAppoinmentDtContainer').datepicker(
            {
                autoclose: 1,
                todayHighlight: 1,
                startDate: new Date()
        ...
        $('#frm_apppointment_vehicle').on('submit', function(e){
            ...
            updateAssessmentNewVehicleAppointmentDisposal(formData);
        });
