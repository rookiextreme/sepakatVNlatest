...
<div class="table-responsive">
    <table class="table-custom no-footer stripe">
        <thead>
            @if($hasAssessmentDisposalDetail->hasStatus->code == '01')
                <th>
            ...
            <th>Harga Asal</th>
            <th>Usia</th>
            @if($hasAssessmentDisposalDetail->hasStatus->code == '08')
            <th>Senarai Semak</th>
            @endif
        </thead>
        <tbody>
            @foreach ($vehicleList as $index => $vehicle)
                <tr class="vehicle">
                    ...
                    <td class="caps">
                        ...
                        @if($OwnImage > 0)
                            <button type="button" class="btn cux-btn small" data-bs-toggle="modal" data-bs-target="#vehicleImagesModal"
                                onclick="loadVehicleImages({{$vehicle->id}}, '06')"><i class="fa fa-image-polaroid"></i> </button>
                    ...
                    <td>{{$vehicle->hasVehicleBrand ? $vehicle->hasVehicleBrand->name : '-'}} {{$vehicle->model_name}}</td>
                    <td>{{Carbon\Carbon::parse($vehicle->purchase_dt)->format("d M Y")}}</td>
                    <td>RM {{number_format($vehicle->original_price, 2)}}</td>
                    <td>{{Carbon\Carbon::now()->diffInYears(Carbon\Carbon::parse($vehicle->purchase_dt))}}</td>
                    @if($hasAssessmentDisposalDetail->hasStatus->code == '08')
                    <td>
                        <button type="button" class="btn cux-btn small" data-url="{{route('assessment.checklist-disposal-report', ['assessment_type_code' => '06', 'vehicle_id' => $vehicle->id])}}" onclick="viewCheckList(this)">Lihat Senarai</button>
                    </td>
                    @endif
                </tr>
            @endforeach
...
{{$vehicleList->links('pagination.ajax-default', [
   'targetDivList' =>  '#assessment_disposal_vehicle',
   'params' => $params
])}}
