@php
session()->put('disposal_current_detail_id', null);
$AccessAssessment = auth()->user()->vehicle('02', '05');
...
$disposalAssessmentDAO = new AssessmentDisposalDAO();
$currentReq = new Request();
$status_code = Request('status_code') ? Request('status_code') : 'all_inprogress';
$search = Request('search') ? Request('search') : null;
$schema = "assessment";
$AssessmentStatus = DB::table($schema.'.assessment_disposal AS a')
...
$totalCompleted = 0;
$totalRejected = 0;
$myjob_total = $disposalAssessmentDAO->list($currentReq->merge(
    [
        'status_code' => 'my_job',
        'mode' => 'count'
    ]));
...
<body class="content">
            ...
            <div class="col-xl-6 col-lg-8 col-md-8 col-sm-6 col-12">
                <div class="dropdown inline" style="display: inline;">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                        ...
                        <li><a class="dropdown-item {{$status_code == 'all_inprogress' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.disposal.list', ['status_code' =>  'all_inprogress'])}}')">Dalam Proses
                            @if($totalProcess > 0)<span class="badge bg-secondary">{{$totalProcess}}</span>@endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == 'my_job' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.disposal.list', ['status_code' =>  'my_job'])}}')">Tindakan Saya 
                            @if($myjob_total > 0)<span class="badge bg-secondary">{{$myjob_total}}</span>@endif
                        </a></li>
                        <li><hr class="dropdown-divider"></li>
                        <li><h6 class="dropdown-header">Peringkat</h6></li>
                        <li><a class="dropdown-item {{$status_code == '01' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.disposal.list', ['status_code' =>  '01'])}}')">Draf
                            @if ($totalDraft > 0)
                            <span class="badge bg-secondary">{{$totalDraft}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '02' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.disposal.list', ['status_code' =>  '02'])}}')">Temujanji
                            @if ($totalAppointment > 0)
                            <span class="badge bg-secondary">{{$totalAppointment}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '03' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.disposal.list', ['status_code' =>  '03'])}}')">Penilaian
                            @if ($totalAssessment > 0)
                            <span class="badge bg-secondary">{{$totalAssessment}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '04' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.disposal.list', ['status_code' =>  '04'])}}')">Semakan
                            @if ($totalVerification > 0)
                            <span class="badge bg-secondary">{{$totalVerification}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '05' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.disposal.list', ['status_code' =>  '05'])}}')">Kelulusan
                            @if ($totalApproval > 0)
                        ...
                        <li><a class="dropdown-item {{$status_code == '08' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.disposal.list', ['status_code' =>  '08'])}}')">Selesai
                            @if ($totalCompleted > 0)<span class="badge bg-secondary">{{$totalCompleted}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '06' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.disposal.list', ['status_code' =>  '06'])}}')">Batal
                ...
                <div class="btn-group">
                    ...
                    <button type="button" onclick="openPgInFrame('{{route('assessment.disposal.register')}}')" aria-readonly=""
                    ...
                    {{-- @if ($AccessAssessment->mod_fleet_d == 1) --}}
                        @if($status_code != '08' || auth()->user()->isAdmin())
                            @if($status_code == '06' || $status_code == '01' || auth()->user()->isAdmin())
                                <button data-bs-toggle="modal" data-bs-target="#assessmentDisposalDelModal" disabled
                            class="btn cux-btn bigger delete_all" type="button"><i class="fal fa-trash-alt"></i></button>
                            @else
                                <button data-bs-toggle="modal" data-bs-target="#assessmentDiposalCancelModal" disabled class="btn cux-btn bigger delete_all" type="button"><i class="fal fa-eraser"></i></button>
                            @endif
                        @endif
                    {{-- @endif --}}
                </div>
        ...
        <div class="table-responsive">
            <table class="table-custom no-footer stripe">
                ...
                <tbody id="sortable">
                    ...
                    @php
                        switch ($disposal->hasStatus->code) {
                            case '01':
                                $tab = 1;
                                break;
                            case '02':
                                // if($TaskFlowAccessAssessmentDisposal->mod_fleet_appointment){
                                //     $tab = 3;
                                // }
                                $tab = 3;
                                break;
                            case '03':
                                // if($TaskFlowAccessAssessmentDisposal->mod_fleet_verify){
                                //     $tab = 4;
                                // }
                                $tab = 4;
                                break;
                        ...
                        if(auth()->user()->isAssistEngineerAssessment()){
                                if($disposal->hasStatus->code == '02'){
                                    $tab = 3;
                                }else{
                                    $tab = 6;
                                }
                            }else if(auth()->user()->isEngineerAssessment()){
                                if($disposal->hasStatus->code == '08'){
                                    $tab = 8;
                                }else{
                                    $tab = 7;
                                }
                    ...
                    @endphp
                     <tr>
                        <td><input class="form-check-input" name="chkdel" id="chkdel" type="checkbox" value="{{$disposal->id}}"></td>
                        <td class="col-edit">
                            {{-- <button type="button" class="btn" onClick="openPgInFrame('{{route('assessment.disposal.register', [ 'id' =>  $disposal->id, 'tab' => $tab ])}}')"><i class="fa fa-pencil-alt"></i></button> --}}
                            <br>
                            <div class="btn-group">
                                    <button class='btn cux-btn' onclick="parent.openPgInFrame('{{route('assessment.disposal.register', [ 'id' =>  $disposal->id, 'tab' => $tab, 'status_code' => $status_code ])}}')"><i class="fa fa-pencil-alt"></i></button>
                                    <button class='btn cux-btn' onclick="openPgInFrame('{{route('report.report_assessment_stmt', [ 'id' =>  $disposal->id, 'workshop_id' => $disposal->hasWorkshop ? $disposal->hasWorkshop->id: null, 'ref_number' =>  $disposal->ref_number, 'assessment_type' => 'disposal'])}}')"><i class="fa fa-print text-success"></i></button>
                        ...
                        <td><a href="{{route('assessment.disposal.register', [ 'id' =>  $disposal->id, 'tab' => $tab, 'status_code' => $status_code ])}}">{{$disposal->ref_number}}</a></td>
                        <td><a href="{{route('assessment.disposal.register', [ 'id' =>  $disposal->id, 'tab' => $tab, 'status_code' => $status_code ])}}">{{$disposal->applicant_name}}</a></td>
                        <td>{{$disposal->phone_no}}</td>
                        <td class="caps">{{$disposal->hasAgency ? $disposal->hasAgency->desc : ''}}</td>
                        <td class="caps">{{$disposal->department_name}}</td>
                        <td class="caps text-end">{{$disposal->hasTotalVeh()}}</td>
                        <td class="caps">{{$disposal->hasStatus->desc}}</td>
                        <td class="caps">{{\Carbon\Carbon::parse($disposal->created_at)->format('d M Y')}}<br/><small>{{\Carbon\Carbon::parse($disposal->created_at)->format('g:i A')}}</small></td>
                    </tr>
                    @endforeach
            ...
            {{$disposals->links('pagination.default')}}
        </div>
    </div>


    {{--<div class="modal fade modal-asking" id="assessmentDisposalDelModal" data-bs-backdrop="static" data-bs-keyboard="false"
        tabindex="-1" aria-labelledby="assessmentDisposalDelModalLabel" aria-hidden="true">
    ...
    <div class="modal fade modal-asking" id="assessmentDisposalDelModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="verifyApplicationModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form class="row" id="frm_examination">
                ...
                <div class="modal-content awas" style="height:250px;background-image: url({{asset('my-assets/img/awas.png')}});">
                    <div class="modal-body pt-5">
                        <div class="asking">
                            Adakah anda benar mahu menghapuskan permohonan ini?
                        </div>
                        <input type="hidden" name="section" value="examination">
                    </div>
                    <div class="modal-footer justify-content-start">
                        <button class="btn btn-module" type="button" id="remove" onclick="remove()">Pasti</button>
                        <button type="button" class="btn btn-link" id="reload_remove" onclick="reloadPage()" data-bs-dismiss="modal">Tutup</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="modal fade modal-asking" id="assessmentDiposalCancelModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="assessmentDiposalCancelModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content awas" style="height:250px;background-image: url({{asset('my-assets/img/awas.png')}});">
                <div class="modal-body pt-5">
                    <div class="asking">
                        Adakah anda benar mahu<br/>membatalkan permohonan ini?
                    </div>
                    <input type="hidden" name="section" value="examination">
                </div>
                <div class="modal-footer justify-content-start">
                    <button type="button" class="btn btn-module" id="cancel" onclick="cancel()">Pasti</button>
                    <button type="button" class="btn btn-link" id="reload_cancel" onclick="reloadPage()" data-bs-dismiss="modal">Tutup</button>
                </div>
            </div>
        </div>
    </div>
    {{--<div class="modal fade modal-asking" id="assessmentDiposalCancelModal" data-bs-backdrop="static" data-bs-keyboard="false"
        tabindex="-1" aria-labelledby="assessmentDiposalCancelModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <h3 class="title"></h3>
    ...
    <script>
        ...
        function searching(value){
            ...
            if(keycode ==  13 || !value){
                window.location.href = "{{route('assessment.disposal.list', ['status_code' => $status_code ])}}&search="+value;
            }
        }
        ...
        function remove(){
            $('#assessmentDisposalDelModal #remove').hide();
            $('#assessmentDisposalDelModal #close_remove').hide();
            $.post("{{route('assessment.disposal.delete')}}", {
                ids: ids,
                '_token': '{{ csrf_token() }}'
            },  function(result){
                if(result.code == '200') {
                    $('#assessmentDisposalDelModal .sub-title').text(result.message);
                }
                $('#assessmentDisposalDelModal #reload_remove').show();
            })
        }

        function cancel(){
            $('#assessmentDiposalCancelModal #cancel').hide();
            $('#assessmentDiposalCancelModal #close_cancel').hide();
            $.post("{{route('assessment.disposal.cancel')}}", {
                ids: ids,
                '_token': '{{ csrf_token() }}'
            },  function(result){
                if(result.code == '200') {
                    $('#assessmentDiposalCancelModal .sub-title').text(result.message);
                }
                $('#assessmentDiposalCancelModal #reload_cancel').show();
            })
        }
