                    ...
                    <td class="caps">
                        ...
                        @if($OwnImage > 0)
                            <button type="button" class="btn cux-btn small" data-bs-toggle="modal" data-bs-target="#vehicleImagesModal"
                                onclick="loadVehicleImages({{$vehicle->id}}, '04')"><i class="fa fa-image-polaroid"></i> </button>

                        @else
                            Tiada gambar dimuatnaik
                        @endif
                    </td>
                    <td>{{$vehicle->manufacture_year ? $vehicle->manufacture_year : '-'}}</td>
                    <td class="caps">{{$vehicle->registration_vehicle_dt ? Carbon\Carbon::parse($vehicle->registration_vehicle_dt)->format("d M Y") : '-'}}</td>
