...
<div class="table-responsive pt-0">
    <table id="fleet-ls" class="table-custom stripe">
        ...
        <tbody>
            @foreach ($vehicleList as $index => $vehicle)
                <tr>
                    ...
                    {{-- <td class="pb-0"><div style="font-size: 14px">{{$page > 1  ?  (($page-1) * $limit) + ($vehicleList->firstItem() + $index) : ($vehicleList->firstItem() + $index)}}</div><input type="hidden" name="vehicle_id[{{$loop->index}}]" value="{{$vehicle->id}}"></td> --}}
                    ...
                    <td class="pb-0" style="padding-top:3px;padding-left:4px;padding-right:4px;padding-bottom:0px;width:270px">
                        @if($hasAssessmentCurrvalueDetail->hasStatus->code == '02')
                        <select data-vehicle-id="{{$vehicle->id}}" name="foremen_id" class="foremen" id="foremen_id_{{$vehicle->id}}">
                        ...
                        @else
                            {{$vehicle->foremenBy ? $vehicle->foremenBy->username : '-'}}
                        @endif
                    </td>
        ...
        {{-- <tfoot>
            <th colspan="7" class="text-end pt-3">Jumlah <small>RM</small></th>
            <th style="text-align:right;width:100px"><span id="grandtotal" style="color:#000000">{{ number_format((float)($totalPrice + $currentTotal), 2, '.', '')}}</span></th>
        </tfoot> --}}
    </table>
</div>
...
<!-- Modal -->
   ...
   {{$vehicleList->links('pagination.ajax-default', [
       'targetDivList' =>  '#assessment_currvalue_vehicle_appointment',
       'params' => $params
   ])}}

<script type="text/javascript">

    // var prevTotal = {{$currentTotal ?: 0}};
    // var ids = [];
    ...
    //         sum += +$(this).val();
    //     });
    //     $('#grandtotal').text(parseFloat(sum+prevTotal).toFixed(2));
    // }
    ...
    updatePrice = function(self){
        ...
        $.post('{{route('assessment.currvalue.vehicle-appointment-updatePrice')}}', {
            '_token': crf,
    ...
    assignToFormen = function(vehicle_id, formen_id){
        $.post("{{route('assessment.currvalue.vehicle-appointment-assign')}}", {
            vehicle_id: vehicle_id,
            user_id: formen_id,
            assign_to: 'formen',
            '_token': '{{ csrf_token() }}'
