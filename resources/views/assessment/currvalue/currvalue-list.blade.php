@php
session()->put('currvalue_current_detail_id', null);
$AccessAssessment = auth()->user()->vehicle('02', '03');
...
$currAssessmentDAO = new AssessmentNewDAO();
$currentReq = new Request();

$status_code = Request('status_code') ? Request('status_code') : 'all_inprogress';
$search = Request('search') ? Request('search') : null;
$schema = "assessment";
$AssessmentStatus = DB::table($schema.'.assessment_currvalue AS a')
...
$totalCompleted = 0;
$totalRejected = 0;
$myjob_total = $currAssessmentDAO->list($currentReq->merge(
    [
        'status_code' => 'my_job',
        'mode' => 'count'
    ]));
...
<body class="content">
            ...
            <div class="col-xl-6 col-lg-8 col-md-8 col-sm-6 col-12">
                <div class="dropdown inline" style="display: inline;">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                        ...
                        <li><a class="dropdown-item {{$status_code == 'all_inprogress' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.currvalue.list', ['status_code' =>  'all_inprogress'])}}')">Dalam Proses
                            @if($totalProcess > 0)<span class="badge bg-secondary">{{$totalProcess}}</span>@endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == 'my_job' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.currvalue.list', ['status_code' =>  'my_job'])}}')">Tindakan Saya
                            @if($myjob_total > 0)<span class="badge bg-secondary">{{$myjob_total}}</span>@endif
                        </a></li>
                        <li><hr class="dropdown-divider"></li>
                        <li><h6 class="dropdown-header">Peringkat</h6></li>
                        <li><a class="dropdown-item {{$status_code == '01' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.currvalue.list', ['status_code' =>  '01'])}}')">Draf
                            @if ($totalDraft> 0)
                            <span class="badge bg-secondary">{{$totalDraft}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '02' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.currvalue.list', ['status_code' =>  '02'])}}')">Temujanji
                            @if ($totalAppointment> 0)
                            <span class="badge bg-secondary">{{$totalAppointment}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '03' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.currvalue.list', ['status_code' =>  '03'])}}')">Penilaian
                            @if ($totalAssessment> 0)
                            <span class="badge bg-secondary">{{$totalAssessment}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '04' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.currvalue.list', ['status_code' =>  '04'])}}')">Semakan
                            @if ($totalVerification> 0)
                            <span class="badge bg-secondary">{{$totalVerification}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '05' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.currvalue.list', ['status_code' =>  '05'])}}')">Kelulusan
                            @if ($totalApproval> 0)
                        ...
                        <li><a class="dropdown-item {{$status_code == '08' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.currvalue.list', ['status_code' =>  '08'])}}')">Selesai
                            @if ($totalCompleted> 0)<span class="badge bg-secondary">{{$totalCompleted}}</span>
                            @endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '06' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.currvalue.list', ['status_code' =>  '06'])}}')">Batal
                ...
                <div class="btn-group">
                    ...
                    <button type="button" onclick="openPgInFrame('{{route('assessment.currvalue.register')}}')" aria-readonly=""
        ...
        <div class="table-responsive">
            <table class="table-custom no-footer stripe">
                ...
                <tbody id="sortable">
                    ...
                    @php
                        switch ($currvalue->hasStatus->code) {
                            case '01':
                                $tab = 1;
                                break;
                            case '02':
                                // if($TaskFlowAccessAssessmentCurrValue->mod_fleet_appointment){
                                //     $tab = 3;
                                // }
                                $tab = 3;
                                break;
                            case '03':
                                // if($TaskFlowAccessAssessmentCurrValue->mod_fleet_verify){
                                //     $tab = 4;
                                // }
                                $tab = 4;
                                break;
                        ...
                        if(auth()->user()->isAssistEngineerAssessment()){
                                if($currvalue->hasStatus->code == '02'){
                                    $tab = 3;
                                }else{
                                    $tab = 6;
                                }
                            }else if(auth()->user()->isEngineerAssessment()){
                                if($currvalue->hasStatus->code == '08'){
                                    $tab = 8;
                                }else{
                                    $tab = 7;
                                }
                    ...
                    @endphp
                     <tr>
                        <td><input class="form-check-input" name="chkdel" id="chkdel" type="checkbox" value="{{$currvalue->id}}"></td>
                        <td class="col-edit">
                            {{-- <button type="button" class="btn" onClick="openPgInFrame('{{route('assessment.currvalue.register', [ 'id' =>  $currvalue->id, 'tab' => $tab ])}}')"><i class="fa fa-pencil-alt"></i></button> --}}
                            <br>
                            <div class="btn-group">
                                    <button class='btn cux-btn' onclick="parent.openPgInFrame('{{route('assessment.currvalue.register', [ 'id' =>  $currvalue->id, 'tab' => $tab, 'status_code' => $status_code ])}}')"><i class="fa fa-pencil-alt"></i></button>
                                    <button class='btn cux-btn' onclick="openPgInFrame('{{route('report.report_assessment_stmt', [ 'id' =>  $currvalue->id, 'workshop_id' => $currvalue->hasWorkshop ? $currvalue->hasWorkshop->id: null, 'ref_number' =>  $currvalue->ref_number, 'assessment_type' => 'currvalue'])}}')"><i class="fa fa-print text-success"></i></button>
                        ...
                        <td><a href="{{route('assessment.currvalue.register', [ 'id' =>  $currvalue->id, 'tab' => $tab, 'status_code' => $status_code ])}}">{{$currvalue->ref_number}}</a></td>
                        <td><a href="{{route('assessment.currvalue.register', [ 'id' =>  $currvalue->id, 'tab' => $tab, 'status_code' => $status_code ])}}">{{$currvalue->applicant_name}}</a></td>
                        <td>{{$currvalue->phone_no}}</td>
                        <td class="caps">{{$currvalue->hasAgency->desc}}</td>
                        <td class="caps">{{$currvalue->department_name}}</td>
                        <td class="caps text-end">{{$currvalue->hasTotalVeh()}}</td>
                        <td class="caps">{{$currvalue->hasStatus->desc}}</td>
                        <td class="caps">{{\Carbon\Carbon::parse($currvalue->created_at)->format('d M Y')}}<br/><small>{{\Carbon\Carbon::parse($currvalue->created_at)->format('g:i A')}}</small></td>
                    </tr>
                    @endforeach
                    @if (count($currvalues) == 0)
                    <tr>
                        <td colspan="10" class="no-record"><i class="fas fa-info-circle"></i> Tiada rekod</td>
                    </tr>
                    @endif
                </tbody>
            </table>
            {{$currvalues->links('pagination.default')}}
        </div>
    </div>
    ...
    <script>
        ...
        function searching(value){
            ...
            if(keycode ==  13 || !value){
                window.location.href = "{{route('assessment.currvalue.list', ['status_code' => $status_code ])}}&search="+value;
            }
        }
        ...
        function remove(){
            ...
            $.post("{{route('assessment.currvalue.delete')}}", {
                ids: ids,
                '_token': '{{ csrf_token() }}'
            },  function(result){
                if(result.code == '200') {
        ...
        function cancel(){
            ...
            $.post("{{route('assessment.currvalue.cancel')}}", {
                ids: ids,
                '_token': '{{ csrf_token() }}'
            },  function(result){
                if(result.code == '200') {
