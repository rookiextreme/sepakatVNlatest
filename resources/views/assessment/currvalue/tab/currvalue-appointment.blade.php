...
<form class="row" id="frm_apppointment_vehicle" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col-xl-3 col-lg-3 col-md-4 col-12">
            <div class="form-group">
                ...
                @else
                    <div class="txt_data ass_gov">{{$detail->hasWorkshop->desc}}</div>
                @endif
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-xl-2 col-lg-2 col-md-3 col-sm-4 col-12">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" onchange="this.value = this.value.toUpperCase()" name="bpk_no" id="bpk_no" value="{{$detail->hasBpkNo->code}}">
                </div>
            </div>
            <div class="col-xl-2 col-lg-2 col-md-1 col-sm-2 col-12">
                <div class="form-group">
                    <label for="" class="form-label">No. PTJ</label>
                    <input type="text" id="ptj_no" name="ptj_no" class="input-sm form-control" placeholder="No PTJ" value="{{$detail->no_ptj}}">
                </div>
            </div>
    ...
    <fieldset>
        ...
        @if((auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerAssessment()))
            <div class="col-xl-12 col-lg-12 col-md-12" id="assessment_currvalue_vehicle_appointment">
    ...
    @endif
    @if(auth()->user()->isAssistEngineerAssessment())
        <div class="col-md-12" id="assessment_currvalue_vehicle_appointment">
...
<div class="modal fade modal-asking" id="assessmentCurrvalueVehicleDelModal" data-bs-backdrop="static" data-bs-keyboard="false"
    tabindex="-1" aria-labelledby="assessmentCurrvalueVehicleDelModalLabel" aria-hidden="true">
...
function checkJob(assessment_id) {
    ...
    if(totalprice == 0){
        ...
        if(bpk_no != ''){
            window.open("{{route('jasperReport')}}?format=pdf&title=BorangPesananKerja&report_name=borang_pesanan_kerja_harga_semasa&assessment_currvalue_id="+assessment_id+"&ptj_no="+ptj_no);
        }else{
    ...
    loadAssessmentCurrvalueVehicleAppointment = function() {
        $.get("{{ route('assessment.currvalue.vehicle-appointment.list') }}", function(result){
            $('#assessment_currvalue_vehicle_appointment').html(result);
        });
    }

    updateAssessmentCurrvalueVehicleAppointment =  function(data){
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // loadAssessmentCurrvalueVehicleAppointment();
    ...
    function remove(){
        $('#assessmentCurrvalueVehicleDelModal #remove').hide();
        $('#assessmentCurrvalueVehicleDelModal #close').hide();
        $.post("{{route('assessment.currvalue.vehicle.cancel')}}", {
            ids: ids,
            '_token': '{{ csrf_token() }}'
        },  function(result){
            $('#assessmentCurrvalueVehicleDelModal').modal('hide');
            loadAssessmentCurrvalueVehicleAppointment();
        })
    }
    ...
    $(document).ready(function(){
        ...
        $('#frm_apppointment_vehicle').on('submit', function(e){
            ...
            updateAssessmentCurrvalueVehicleAppointment(formData);
        });
