@php
session()->put('safety_current_detail_id', null);
$AccessAssessment = auth()->user()->vehicle('02', '02');
...
$safetyAsessmentDAO = new AssessmentSafetyDAO();
$currentReq = new Request();

$status_code = Request('status_code') ? Request('status_code') : 'all_inprogress';
$search = Request('search') ? Request('search') : null;
$schema = "assessment";
$AssessmentStatus = DB::table($schema.'.assessment_safety AS a')
...
$totalCompleted = 0;
$totalRejected = 0;
$myjob_total = $safetyAsessmentDAO->list($currentReq->merge(
    [
        'status_code' => 'my_job',
        'mode' => 'count'
    ]));
...
<body class="content">
            ...
            <div class="col-xl-6 col-lg-8 col-md-8 col-sm-6 col-12">
                <div class="dropdown inline" style="display: inline;">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                        ...
                        <li><a class="dropdown-item {{$status_code == 'all_inprogress' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.safety.list', ['status_code' =>  'all_inprogress'])}}')">Dalam Proses
                            @if($totalProcess >= 1)<span class="badge bg-secondary">{{$totalProcess}}</span>@endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == 'my_job' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.safety.list', ['status_code' =>  'my_job'])}}')">Tindakan Saya
                            @if($myjob_total >= 1)<span class="badge bg-secondary">{{$myjob_total}}</span>@endif
                        </a></li>
                        <li><hr class="dropdown-divider"></li>
                        <li><h6 class="dropdown-header">Peringkat</h6></li>
                        <li><a class="dropdown-item {{$status_code == '01' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.safety.list', ['status_code' =>  '01'])}}')">Draf
                            @if($totalDraft >= 1)<span class="badge bg-secondary">{{$totalDraft}}</span>@endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '02' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.safety.list', ['status_code' =>  '02'])}}')">Temujanji
                            @if($totalAppointment >= 1)<span class="badge bg-secondary">{{$totalAppointment}}</span>@endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '03' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.safety.list', ['status_code' =>  '03'])}}')">Penilaian
                            @if($totalAssessment >= 1)<span class="badge bg-secondary">{{$totalAssessment}}</span>@endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '04' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.safety.list', ['status_code' =>  '04'])}}')">Semakan
                            @if($totalVerification >= 1)<span class="badge bg-secondary">{{$totalVerification}}</span>@endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '05' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.safety.list', ['status_code' =>  '05'])}}')">Kelulusan
                            @if($totalApproval >= 1)<span class="badge bg-secondary">{{$totalApproval}}</span>@endif
                        </a></li>
                        <li><hr class="dropdown-divider"></li>
                        <li><h6 class="dropdown-header">Arkib</h6></li>
                        <li><a class="dropdown-item {{$status_code == '08' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.safety.list', ['status_code' =>  '08'])}}')">Selesai
                            @if($totalCompleted >= 1)<span class="badge bg-secondary">{{$totalCompleted}}</span>@endif
                        </a></li>
                        <li><a class="dropdown-item {{$status_code == '06' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('assessment.safety.list', ['status_code' =>  '06'])}}')">Batal
                            @if ($totalRejected > 1)<span class="badge bg-secondary">{{$totalRejected}}</span>
                            @endif
                        </a></li>
                    </ul>
                </div>
                <div class="btn-group">
                    ...
                    <a onclick="openPgInFrame('{{route('assessment.safety.register')}}')" aria-readonly=""
                    ...
                    @if ($AccessAssessment->mod_fleet_d == 1)
                        @if($status_code != '08' || auth()->user()->isAdmin())
                            @if($status_code == '06' || $status_code == '01' || auth()->user()->isAdmin())
                                <button data-bs-toggle="modal" data-bs-target="#assessmentSafetyDelModal" disabled
                            class="btn cux-btn bigger delete_all" type="button"><i class="fal fa-trash-alt"></i> Hapus</button>
                            @else
                                <button data-bs-toggle="modal" data-bs-target="#AssessmentSafetyCancelModal" disabled class="btn cux-btn bigger delete_all" type="button"><i class="fal fa-eraser"></i> Batal</button>
                            @endif
                        @endif
                    @endif --}}
                    @if ($AccessAssessment->mod_fleet_c == 1)
                    ...
                    <button type="button" onclick="openPgInFrame('{{route('assessment.safety.register')}}')" aria-readonly=""
                    ...
                    {{-- @if ($AccessAssessment->mod_fleet_d == 1) --}}
                        @if($status_code != '08' || auth()->user()->isAdmin())
                            ...
                            @else
                                <button data-bs-toggle="modal" data-bs-target="#AssessmentSafetyCancelModal" disabled class="btn cux-btn bigger delete_all" type="button"><i class="fal fa-eraser"></i></button>
                            @endif
                        @endif
                    {{-- @endif --}}
                </div>
        ...
        <div class="table-responsive">
            <table class="table-custom no-footer stripe">
                ...
                <tbody id="sortable">
                    ...
                    @php
                        switch ($safety->hasStatus->code) {
                            case '01':
                                $tab = 1;
                                break;
                            case '02':
                                // if($TaskFlowAccessAssessmentSafety->mod_fleet_appointment){
                                //     $tab = 3;
                                // }
                                $tab = 3;
                                break;
                            case '03':
                                // if($TaskFlowAccessAssessmentSafety->mod_fleet_verify){
                                //     $tab = 4;
                                // }
                                $tab = 4;
                                break;
                        ...
                        if(auth()->user()->isAssistEngineerAssessment()){
                                if($safety->hasStatus->code == '02'){
                                    $tab = 3;
                                }else{
                                    $tab = 6;
                                }
                            }else if(auth()->user()->isEngineerAssessment()){
                                if($safety->hasStatus->code == '08'){
                                    $tab = 8;
                                }else{
                                    $tab = 7;
                                }
                    ...
                    @endphp
                     <tr>
                        <td><input class="form-check-input" name="chkdel" id="chkdel" type="checkbox" value="{{$safety->id}}"></td>
                        <td class="col-edit">
                            {{-- <button type="button" class="btn" onClick="openPgInFrame('{{route('assessment.safety.register', [ 'id' =>  $safety->id, 'tab' => $tab ])}}')"><i class="fa fa-pencil-alt"></i></button> --}}
                            <br>
                            <div class="btn-group">
                                    <button class='btn cux-btn' onclick="parent.openPgInFrame('{{route('assessment.safety.register', [ 'id' =>  $safety->id, 'tab' => $tab ])}}')"><i class="fa fa-pencil-alt"></i></button>
                                    <button class='btn cux-btn' onclick="openPgInFrame('{{route('report.report_assessment_stmt', [ 'id' =>  $safety->id, 'workshop_id' => $safety->hasWorkshop ? $safety->hasWorkshop->id: null, 'ref_number' =>  $safety->ref_number, 'assessment_type' => 'safety'])}}')"><i class="fa fa-print text-success"></i></button>
                        ...
                        <td><a href="{{route('assessment.safety.register', [ 'id' =>  $safety->id, 'tab' => $tab, 'status_code' =>$status_code ])}}">{{$safety->ref_number}}</a></td>
                        <td><a href="{{route('assessment.safety.register', [ 'id' =>  $safety->id, 'tab' => $tab, 'status_code' =>$status_code])}}">{{$safety->applicant_name}}</a></td>
                        <td>{{$safety->phone_no}}</td>
                        <td class="caps">{{$safety->hasAgency ? $safety->hasAgency->desc : '-'}}</td>
                        <td class="caps">{{$safety->department_name}}</td>
                        <td class="caps text-end">{{$safety->hasTotalVeh()}}</td>
                        <td class="caps">{{$safety->hasStatus->desc}}</td>
                        <td class="caps">{{\Carbon\Carbon::parse($safety->created_at)->format('d M Y')}}<br/><small>{{\Carbon\Carbon::parse($safety->created_at)->format('g:i A')}}</small></td>
                    </tr>
                    @endforeach
                    @if (count($safetys) == 0)
                    <tr class="no-record">
            ...
            {{$safetys->links('pagination.default')}}
        </div>
    </div>


    <div class="modal fade" id="AssessmentSafetyDelModal" data-bs-backdrop="static" data-bs-keyboard="false"
        tabindex="-1" aria-labelledby="AssessmentSafetyDelModalLabel" aria-hidden="true">
    ...
    <div class="modal fade" id="AssessmentSafetyCancelModal" data-bs-backdrop="static" data-bs-keyboard="false"
        tabindex="-1" aria-labelledby="AssessmentSafetyCancelModalLabel" aria-hidden="true" style="margin-top:10vh">
        <div class="modal-dialog modal-asking">
            <div class="modal-content awas p-3" style="background-image: url({{asset('my-assets/img/awas.png')}});">
                <div class="modal-body">
                    <div class="attention">Batalkan Permohonan?</div>
                </div>
                <div class="modal-footer float-start">
    ...
    <script>
        ...
        function searching(value){
            ...
            if(keycode ==  13 || !value){
                window.location.href = "{{route('assessment.safety.list', ['status_code' => $status_code ])}}&search="+value;
            }
        }
        ...
        function remove(){
            $('#AssessmentSafetyDelModal #remove').hide();
            $('#AssessmentSafetyDelModal #close').hide();
            $.post("{{route('assessment.safety.delete')}}", {
                ids: ids,
                '_token': '{{ csrf_token() }}'
            },  function(result){
                if(result.code == '200') {
                    $('#AssessmentSafetyDelModal .sub-title').text(result.message);
                }
                $('#AssessmentSafetyDelModal #reload').show();
            })
        }

        function cancel(){
            $('#AssessmentSafetyCancelModal #cancel').hide();
            $('#AssessmentSafetyCancelModal #close_cancel').hide();
            $.post("{{route('assessment.safety.cancel')}}", {
                ids: ids,
                '_token': '{{ csrf_token() }}'
            },  function(result){
                if(result.code == '200') {
                    $('#AssessmentSafetyCancelModal .sub-title').text(result.message);
                }
                $('#AssessmentSafetyCancelModal #reload_cancel').show();
            })
        }
