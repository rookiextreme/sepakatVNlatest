...
<form id="frm_vehicle" enctype="multipart/form-data">
    @csrf
    ...
    @endif
    <div class="col-xl-12 col-lg-12 col-md-12 mb-2" id="assessment_safety_vehicle"></div>
    <hr/>
      ...
      <div class="modal-content">
        ...
        <form id="frmAssignAssessmentVehicleAppointment" action="">
            ...
            <div class="modal-footer justify-content-between">
                ...
                <button type="button" onclick="loadAssessmentSafetyVehicle()" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
            </div>
        </form>
      </div>
    </div>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xl-2 col-lg-2 col-md-3">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="model_name" id="model_name" placeholder="cth: X70" xmodel_name onchange="this.value = this.value.toUpperCase()" maxlength="30">
                                <div class="hasErr"></div>
                            </div>
                        </div>
                        <div class="col-xl-2 col-lg-3 col-md-3">
                        ...
                        <div class="col-xl-2 col-lg-3 col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group date" id="registrationVehicleDt" data-date="" data-date-format="dd/mm/yyyy">
                                    <input class="form-control" size="16" id="registrationVehicleDtInput" xregistration_vehicle_dt type="text" value="" placeholder="TARIKH PENDAFTARAN" readonly>
                                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                                    <span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span>
            ...
            <div class="modal-footer justify-content-start">
                <button type="submit" class="btn btn-module add_vehicle">Tambah</button>
                <button type="submit" class="btn btn-module edit_vehicle" style="display:none;">Simpan</button>
                <button type="button" class="btn btn-link" data-bs-dismiss="modal"><i class="fal fa-undo"></i> Batal</button>
...
<script type="text/javascript">
    ...
    loadAssessmentSafetyVehicle = function() {
        let data = {
            'page': currentPage,
        }
        $.get("{{ route('assessment.safety.vehicle.list') }}", data, function(result){
            $('#assessment_safety_vehicle').html(result);
            let totalVehicle = $('#assessment_safety_vehicle tbody tr.vehicle').length;
    ...
    function remove(){
        ...
        $.post("{{route('assessment.safety.vehicle.delete')}}", {
            ids: ids,
            '_token': '{{ csrf_token() }}'
        },  function(response){
            $('#frm_vehicle #response').html(response.message).fadeIn(50).slideUp('slow');
            $('#assessmentSafetyVehicleDelModal').modal('hide');
            loadAssessmentSafetyVehicle();
        })
    }

    submitAssessmentSafetyVehicle = function(data) {

        $('.hasErr').html('');

        $.ajax({
            url: "{{ route('assessment.safety.vehicle.save') }}",
            type: 'post',
            data: data,
            cache: false,
            contentType: false,
            ...
            success: function(response) {
                ...
                $('#frm_vehicle #response').html(response.message).slideDown('slow').delay(4000).slideUp('slow');
    ...
    selectAssessmentSafetyVehicle = function(id){
        $('#assignAssessmentVehicleAppointment').find('#detail_id').val(id);
    }

    assignVehicleAppointment = function(data){
        $.ajax({
            url: "{{ route('assessment.safety.vehicle.assignAppointment') }}",
            type: 'post',
            data: data,
            cache: false,
            contentType: false,
    ...
    function checkExistRegNumber(plate_no){
        ...
        $.ajax({
            url: "{{ route('assessment.safety.vehicle.checkExistRegNumber') }}",
            type: 'post',
            data: {
    ...
    editAssessmentVehicle = function(vehicle_id, functional, self){
        ...
        $('#display_list_sub_category_type').show();
        $.ajax({
            url: '{{Route("assessment.safety.vehicle.getVehicleForm")}}',
            type: 'get',
            data: {
    ...
    $(document).ready(function(){
        ...
        $("#frmAddAssessmentVehicle #brand").select2({
            width: '100%',
            theme: "classic"
        });

        $("#frmAddAssessmentVehicle #category").select2({
            width: '100%',
            theme: "classic"
        });

        $("#frmAddAssessmentVehicle #list_sub_category").select2({
            width: '100%',
            theme: "classic"
        });

        $("#frmAddAssessmentVehicle #list_sub_category_type").select2({
            width: '100%',
            theme: "classic"
        });
        ...
        $('#frmAddAssessmentVehicle').on('submit', function(e) {
            ...
            let formData = new FormData(this);
            formData.append('assessment_safety_id', {{$id}});
            submitAssessmentSafetyVehicle(formData);
