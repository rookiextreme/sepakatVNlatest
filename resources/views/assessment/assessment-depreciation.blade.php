...
<div class="main-content">
    ...
    <section id="kelas" class="tab-content">
        <div class="detailing">
            ...
            <form class="form-submit" id="updateMaklumatKelasKenderaanForm">
                @csrf
                <table>
                    <thead>
                        {{-- <td class="end"></td> --}}
                        ...
                        @foreach ($listArrayKelas[0] as $listData)
                            <input type="hidden" id="vehicleAge" name="vehicleAge" value="{{$listData->age}}">
                            <td class="col-mth header-depreciation">{{$listData->age}}</td>
                        @endforeach
                    ...
                    <tbody>
                        ...
                        @foreach ($listArrayKenderaan as $index_vhc => $listData)
                        {{-- @json($listData[0]) --}}
                        {{-- {{$index_vhc}} --}}
                        <tr>
                            {{-- <td class="end "></td> --}}
                            <td class="row-item" style="text-transform:uppercase">{{$listData[0]->vehicle_name}}</td>
                            @foreach ($listData as $index_class => $data)
                            {{-- {{$index_class}} inindex --}}
                                @php
                                    $mainLoop = $loop->index + 1;
                                @endphp
                                <td class="col-focus" xtype='vehicle'>
                                    <select class="form-control" data-vehicle-id="{{$listData[0]->vehicle_id}}" name="vehicle_class" id="vehicle_class" onchange="updateMaklumatKelasKenderaan(this)">
                                        @foreach ($listKelas as $kelas)
                                            <option data-index-age="{{$mainLoop}}" data-vehicle-class="{{$kelas->kelas_id}}" value="{{$kelas->kelas_id}}" {{$kelas->kelas_id == $data->class_number ? 'selected' : ''}}>{{$kelas->roman_numbering}}</option>
                                        @endforeach
                                    </select>
                                </td>
                            @endforeach
...
<script type="text/javascript">
    ...
    function updateMaklumatKelasKenderaan(self){

        let vehicleModelId = $(self).data('vehicle-id');
        let vehicleAge = $('option:selected', self).data('index-age');
        let vehicleClass = $('option:selected', self).data('vehicle-class');
        ...
        console.log(formData.get('vehicleModelId'));
            ...
            success: function(data){
                console.log("success");
                if(data == 1){
                    $('#response').html('<span class="text-success">Maklumat Berjaya Disimpan</span>').fadeIn(50).fadeOut(5000);

                }else{
                    $('#response').html('<span class="text-warning">Maklumat Gagal Disimpan</span>').fadeIn(50).fadeOut(5000);
