...
<script type="text/javascript">
    function jumpPage(mth){
        var year = $('#year').val();
        window.location.href = "{{route('report.report_evaluation_summary')}}?month="+mth+"&year="+year;
    }
    function changeMth() {
        var txtMth = $('#mth').val();
        switch (txtMth) {
    ...
    <div class="inline thin-underline" style="margin-top:15px;margin-right:30px;padding-bottom:4px;">
        ...
        <ul class="dropdown-menu" style="max-width: 50px !important">
            ...
            @foreach ($yearList as $year)
                <li><a href="{{route('report.report_evaluation_summary', ['year' => $year, 'month' => $month])}}" class="dropdown-item{{Request('year') == $year ? ' active' : ''}}" data-month="{{$year}}" value="{{$year}}">{{$year}}</a></li>
            @endforeach
        </ul>
        ...
        <table class="table-custom hover stripe" style="margin-top:20px">
            <thead>
                <tr>
                    ...
                    <th class="lcal-3 text-end">Semakan Pengujian</th>
                    <th class="lcal-3 text-end">Janaan Perihal Kerosakan</th>
            ...
            <tbody>
                ...
                @endphp
                    <tr>
                                ...
                                @foreach ($appointment as $appoint)
                                    @if ($listing->id == $appoint->id)
                                        @php
                                            ...
                                            $columnAppointment = $appointmenttotal;
                                        @endphp
                                    @else
                                    @endif
                                @endforeach
                                @php
                                    $rowtotal = $rowtotal + $appointmenttotal;
                                @endphp
                        ...
                        <td class="data-col">
                            @if($verification)
                                @foreach ($verification as $verify)
                                    @if ($listing->id == $verify->id)
                                        @php
                                            $verificationtotal = $verificationtotal + 1;
                                            $columnVerify = $verificationtotal;
                                        @endphp
                                    @else
                                    @endif
                                @endforeach
                                @php
                                    $rowtotal =  $rowtotal + $verificationtotal;
                                @endphp
                        ...
                        <td class="data-col">
                            @if($letter)
                                @foreach ($letter as $generateletter)
                                    @if ($listing->id == $generateletter->id)
                                        @php
                                            $lettertotal = $lettertotal + 1;
                                            $columnLetter = $lettertotal;
                                        @endphp
                                    @else
                                    @endif
                                @endforeach
                                @php
                                    $rowtotal =  $rowtotal + $lettertotal;
                                @endphp
                                @if($lettertotal >0)
                                    {{$lettertotal}}
                                @else
            ...
            <tfoot>
                <tr>
                    ...
                    <th class="lcal-3 text-end">{{$columnVerify}}</th>
                    <th class="lcal-3 text-end">{{$columnLetter}}</th>
