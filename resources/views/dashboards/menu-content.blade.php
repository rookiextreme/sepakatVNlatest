...
@php
    ...
    $totalSenggaraanServisMyJob = $jobDAO->list(Request()->merge(
        [
            'status_code' => 'my_job',
            'mode' => 'count'
        ]));
    ...
    if(
        ...
        else if(in_array($roleCode,  ['03', '14','15']) == '03'){
    ...
    <li><img src="{{ asset('my-assets/img/chevron.svg') }}" class="chevron" /> &nbsp;
        ...
        <ul class="subside">
            @if (in_array($roleAccessCode, array('01','03','04')))
                <li><span class="badge bg-spakat" {{$totalPenilaianBaru==0 ? "style=display:none" : ''}}>{{$totalPenilaianBaru}}</span> <a href="javascript:openFromMenu('{{ route('assessment.new.list', ['status_code' => 'my_job']) }}')">Kenderaan Baharu</a></li>
                {{-- @if (!(Auth()->user()->isPublic())) --}}
                <li><span class="badge bg-spakat" {{$totalPenilaianKeselamatan==0 ? "style=display:none" : ""}}>{{$totalPenilaianKeselamatan}}</span><a href="javascript:openFromMenu('{{ route('assessment.safety.list', ['status_code' => 'my_job']) }}')">Keselamatan &amp; Prestasi</a></li>
                {{-- @endif --}}
                <li><span class="badge bg-spakat" {{$totalPenilaianNilaiSemasa==0 ? "style=display:none" : ""}}>{{$totalPenilaianNilaiSemasa}}</span><a href="javascript:openFromMenu('{{ route('assessment.currvalue.list', ['status_code' => 'my_job']) }}')">Harga Semasa</a></li>
                <li><span class="badge bg-spakat" {{$totalPenilaianKemalangan==0 ? "style=display:none" : ""}}>{{$totalPenilaianKemalangan}}</span><a href="javascript:openFromMenu('{{ route('assessment.accident.list', ['status_code' => 'my_job']) }}')">Kemalangan</a></li>
                {{-- @if (!(Auth()->user()->isPublic())) --}}
                <li><span class="badge bg-spakat" {{$totalPenilaianPinjaman==0 ? "style=display:none" : ""}}>{{$totalPenilaianPinjaman}}</span><a href="javascript:openFromMenu('{{ route('assessment.gov_loan.list', ['status_code' => 'my_job']) }}')">Pinjaman Kerajaan</a></li>
                <li><span class="badge bg-spakat" {{$totalPenilaianPelupusan==0 ? "style=display:none" : ""}}>{{$totalPenilaianPelupusan}}</span><a href="javascript:openFromMenu('{{ route('assessment.disposal.list', ['status_code' => 'my_job']) }}')">Pelupusan</a></li>
                {{-- @endif --}}
        ...
        <ul class="subside">
            @if (in_array($roleAccessCode, array('02')))
                ...
                @else
                    ...
                    @endif
                    <li><span class="badge bg-spakat" {{$totalSenggaraanPemeriksaan==0 ? "style=display:none" : ''}}>{{$totalSenggaraanPemeriksaan}}</span> <a href="javascript:openFromMenu('{{ route('maintenance.evaluation.list', [ 'status_code' => 'my_job']) }}')">Pemeriksaan</a></li>
                    <li><span class="badge bg-spakat" {{$totalSenggaraanServisMyJob==0 ? "style=display:none" : ""}}>{{$totalSenggaraanServisMyJob}}</span> <a href="javascript:openFromMenu('{{ route('maintenance.job.list', ['status_code' => 'my_job']) }}')">Servis & Pembaikan</a></li>
        ...
        <ul class="subside">
            ...
            <li><img src="{{ asset('my-assets/img/chevron.svg') }}" class="chevron" /> &nbsp;<a href="#"
                ...
                <ul class="subside">
                    ...
                    <li><a href="javascript:openFromMenu('{{ route('reference.depreciation') }}')">Susut Nilai Kenderaan</a></li>
