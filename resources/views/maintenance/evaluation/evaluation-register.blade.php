...
<body class="content">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="javascript:openPgInFrame('{{ route('maintenance.overview') }}')">Pemeriksaan</a></li>
            <li class="breadcrumb-item">
                <a href="{{ route('maintenance.evaluation.list', ['status_code' => $status_code]) }}">Rekod Pemeriksaan</a>
            </li>
    ...
    <div class="process-stage">
        ...
        @php
            ...
            if (!$detail || $detail && in_array($detail->hasStatus->code, ['01'])) {
                ...
                $stage = 235;
            } else if ($detail && in_array($detail->hasStatus->code, ['11'])) {
                $stage = 320;
        ...
        <div class="stage">
            <div class="num {{$detail && in_array($detail->hasStatus->code, ['01','02','03','04','05','08','09','11']) ? 'done': ''}}">1</div>
            <div class="stagename">Permohonan</div>
        </div>
        <div class="stage">
            <div class="num {{$detail && in_array($detail->hasStatus->code, ['02','03','04','05','08','09','11']) ? 'done': ''}}">2</div>
            <div class="stagename">Temujanji</div>
        </div>
        <div class="stage">
            <div class="num {{$detail && in_array($detail->hasStatus->code, ['03','04','05','08','09','11']) ? 'done': ''}}">3</div>
            <div class="stagename">Penilaian</div>
        </div>
        <div class="stage">
            <div class="num {{$detail && in_array($detail->hasStatus->code, ['05','08','09','11']) ? 'done': ''}}">4</div>
            <div class="stagename">Surat</div>
        </div>
        <div class="stage">
    ...
    <div class="main-content">
        <div class="quick-navigation" data-fixed-after-touch="">
            <div class="wrapper" style="position: relative">
                <ul id="tabActive">
                    ...
                    @if ($detail && $detail && in_array($detail->hasStatus->code, ['01','02','03','04','05','08','09','11']))
                        <li class="cub-tab"  onClick="goTab(this, 'vehicle');" id="tab2">Kenderaan</li>
                    @endif
                    @if ($detail && $detail && in_array($detail->hasStatus->code, ['02','03','04','05','08','09','11']))
                        <li class="cub-tab"  onClick="goTab(this, 'appointment');" id="tab3">Temujanji</li>
                    @endif
                    @if ($detail && $detail && in_array($detail->hasStatus->code, ['03','04','05','08','09','11']) && ($TaskFlowAccessMaintenanceEvaluation->mod_fleet_verify))
                    ...
                    @endif --}}
                    @if ($detail && $detail && in_array($detail->hasStatus->code, ['03', '04','05','08','09','11']) && ($is_myApp || auth()->user()->isAdmin() || auth()->user()->isEngineer() || auth()->user()->isEngineerMaintenance() || auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerMaintenance() || auth()->user()->isForemenMaintenance()))
                        <li class="cub-tab"  onClick="goTab(this, 'generate_letter');" id="tab6">
                    ...
                    @endif
                    {{-- @if ($detail && $detail && in_array($detail->hasStatus->code, ['05','08','09','11']) && $TaskFlowAccessMaintenanceEvaluation->mod_fleet_approval)
        ...
        <section id="application" class="tab-content">
            @include('maintenance.evaluation.tab.evaluation-application')
        </section>
        @if ($detail && $detail && in_array($detail->hasStatus->code, ['01','02','03','04','05','08','09','11']))
        <section id="vehicle" class="tab-content">
            @include('maintenance.evaluation.tab.evaluation-vehicle')
        </section>
        @endif
        @if ($detail && $detail && in_array($detail->hasStatus->code, ['02','03','04','05','08','09','11']))
            <section id="appointment" class="tab-content">
                @include('maintenance.evaluation.tab.evaluation-appointment')
            </section>
        @endif
        @if ($detail && $detail && in_array($detail->hasStatus->code, ['03','04','05','08','09','11']) && (auth()->user()->isForemenMaintenance() || auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerMaintenance()))
            <section id="assessment" class="tab-content">
                @include('maintenance.evaluation.tab.evaluation-assessment')
            </section>
        @endif
        {{-- @if ($detail && $detail && in_array($detail->hasStatus->code, ['03','04','05','08','09','11']) && $TaskFlowAccessMaintenanceEvaluation->mod_fleet_verify)
            <section id="evaluation" class="tab-content">
                @include('maintenance.evaluation.tab.evaluation-evaluation')
            </section>
        @endif --}}
        @if ($detail && $detail && in_array($detail->hasStatus->code, ['03','04','05','08','09','11']) && ($is_myApp || auth()->user()->isAdmin() || auth()->user()->isEngineer() || auth()->user()->isEngineerMaintenance() || auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerMaintenance() || auth()->user()->isForemenMaintenance()))
        <section id="generate_letter" class="tab-content">
            @include('maintenance.evaluation.tab.evaluation-generate-letter')
        </section>
        @endif

        {{-- @if ($detail && $detail && in_array($detail->hasStatus->code, ['05','08','09','11']) && $TaskFlowAccessMaintenanceEvaluation->mod_fleet_approval)
            <section id="approval" class="tab-content">
                @include('maintenance.evaluation.tab.evaluation-approval')
            </section>
        @endif --}}
                ...
                <form class="row" id="frm_verify">
                    ...
                    <div class="modal-content tanda">
                        <div class="modal-body awas" style="background-image: url({{asset('my-assets/img/awas.png')}});">
                            ...
                            <hr>
                            <ul class="checklist">
                                <li><i class="fal fa-check fa-lg"></i> Surat Permohonan Pemeriksaan</li>
                                <li class="mt-3">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <i class="fal fa-check fa-lg"></i> Surat Pelepasan Tanggungan Kenderaan
                                        </div>
                                        <div class="col-md-6">
                                            <span class="ms-2 btn cux-btn back-hailait" onclick="fileUrl('borang_pelepasan_module_penyenggaraan.docx')" ><i class="fal fa-file-pdf"></i> Muat Turun Surat</span>
                                        </div>
                                    </div>
                                </li>
                            </ul>
