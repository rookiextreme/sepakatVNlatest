...
$schema = "maintenance";
$evaluationDAO = new MaintenanceEvaluationDAO();
...
$EvaluationStatus = DB::table($schema.'.evaluation AS a')
    ...
    $totalDraft = DB::table($schema.'.evaluation AS a')
        ...
        ->count();
    $myjob_total = $evaluationDAO->list($currentReq->merge(
    [
        'status_code' => 'my_job',
        'mode' => 'count'
    ]));
    ...
    <div class="sect-top">
        ...
        <div class="dropdown inline" style="display: inline;">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                ...
                <li><a class="dropdown-item {{$status_code == 'my_job' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('maintenance.evaluation.list', ['status_code' =>  'my_job'])}}')">Tindakan Saya
                    @if($myjob_total>0)
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group dropend">
                                        <button type="button" class="btn cux-btn" onClick="openPgInFrame('{{route('maintenance.evaluation.register', [ 'id' =>  $maintenance->id, 'tab' => $tab, 'status_code' => $status_code ])}}')"><i class="fa fa-pencil-alt"></i></button>
                                    </div>
                                </td>
                                @endif
                                <td><a href="{{route('maintenance.evaluation.register', [ 'id' =>  $maintenance->id, 'tab' => $tab, 'status_code' => $status_code])}}">{{$maintenance->ref_number}}</a></td>
                                <td><a href="{{route('maintenance.evaluation.register', [ 'id' =>  $maintenance->id, 'tab' => $tab, 'status_code' => $status_code ])}}">{{$maintenance->hasVehicle() && $maintenance->hasVehicle()->first() ? $maintenance->hasVehicle()->first()->plate_no : ''}}</a></td>
                                <td><a href="{{route('maintenance.evaluation.register', [ 'id' =>  $maintenance->id, 'tab' => $tab, 'status_code' => $status_code ])}}">{{$maintenance->applicant_name}}</a></td>
