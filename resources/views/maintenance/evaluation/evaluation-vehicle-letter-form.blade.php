...
<body class="content">
        ...
        <form action="" id="frm_template_letter">
            ...
            <div class="col-md-12">
                <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-9" style="padding-bottom: 20px"><label for="exam_letter" class="btn cux-btn bigger"><i class="fas fa-image"></i> Muat Naik Surat </label></div>
                        <input onchange="saveSilent(this)" data-lvl="exam_letter" class="form-control d-none" accept="application/pdf" type="file" name="exam_letter" id="exam_letter">
                    ...
                    <div class="row">
                        <div class="col-md-6"  style="padding-bottom: 20px" id="viewExamLetter">
                            @if($detail->hasVehicle->hasTemplateLetter && $detail->hasVehicle->hasTemplateLetter->exam_letter ? $detail->hasVehicle->hasTemplateLetter->exam_letter : '')
                                <button id="exam_letter_preview" onclick="pdfView(this)" class="btn cux-btn small" type="button" preview-url="/storage/public/dokumen/maintenance/evaluation/exam_letter/{{$detail->hasVehicle->hasTemplateLetter->exam_letter}}"style="display:block"> <i class="fa fa-eye"></i> Papar Surat Lampiran  </button>
                           @else
                           @endif
                        </div>
                    </div>
                    <div class="col-md-12">
                        @include('maintenance.evaluation.tab.template.letter', [
                            'detail' => $detail,
    ...
    const submitFormTemplateLetter = function(data){
        parent.startLoading();
        $.ajax({
            url: "{{ route('maintenance.evaluation.vehicle-maintenance.exam.letter.save') }}",
            type: 'post',
            data: data,
            cache: false,
            contentType: false,
    ...
    $(document).ready(function(){
        ...
        $('#exam_letter').on('change', function(e){
            e.preventDefault();
            var output = document.getElementById('exam_letter_preview');
               output.src = URL.createObjectURL(e.target.files[0]);
               const targetPreview = $('#exam_letter_preview');
               $('#exam_letter_preview').show();
               targetPreview.attr('preview-url', output.src);
         });
