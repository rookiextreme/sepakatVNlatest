                ...
                <tr>
                            ...
                            @if($vehicle->hasExamform)
                                <label for="" class="form-label pt-1 ps-2">
                                    ...
                                    @if($vehicle->hasExamform->hasInternalRepairStatus)
                                        <div>
                                            <label class="ps-4 form-label d-inline float-end">
                                            ...
                                            @if(in_array($vehicle->hasExamform->hasMaintenanceJobVehicleStatus->code, ['11']) && in_array($vehicle->hasExamform->hasInternalRepairStatus->code, ['01','02']) && (auth()->user()->isAssistEngineer() || auth()->user()->isAssistEngineerMaintenance()))
                                                <br/><span class="btn btn-sm btn-secondary ms-4" onclick="generateFormEvaluate({{$vehicle->id}}, '02', 4)"> Semak</span>
                                            @endif
                                        </div>
                                    @endif
                                    @if($vehicle->hasExamform->hasExternalRepairStatus)
                                        <div class="mt-2">
                                            ...
                                            <br/><span class="btn btn-sm btn-secondary ms-4" onclick="generateFormEvaluate({{$vehicle->id}}, '02', 5)"> Semak</span>
                                            @endif
                                        </div>
                                    @endif
                            @endif
                    ...
                    <td class="text-center">
                        <div class="input-group flex-nowrap">
                            ...
                            onclick="generateFormEvaluate({{$vehicle->id}}, '02', 1)"> Semak</span>
                            <select data-vehicle-id="{{$vehicle->id}}" name="job_veval_status_code" class="ms-2 job_veval_status_code" id="job_veval_status_code_{{$vehicle->id}}">
...
{{$vehicleList->links('pagination.ajax-default', [
    'targetDivList' =>  '#maintenance_job_vehicle_evaluation',
    'params' => $params
])}}

<script type="text/javascript">

    const updateJobVehicleStatus = function(vehicle_id, status_code){
    ...
    const generateFormEvaluate = function(vehicle_id, maintenance_type_code_selected, tab){
        parent.startLoading();
    ...
    $(document).ready(function(){
        ...
        $('.job_veval_status_code').select2({
            ...
            let code = this.value;
            updateJobVehicleStatus(aVehicleId, code);
            console.log(this);
        });
