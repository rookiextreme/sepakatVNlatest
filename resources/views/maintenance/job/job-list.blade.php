...
if($TaskFlowAccessMaintenanceJob->mod_fleet_approval){
    ...
    $totalProcurementEvaluation = DB::table($schema.'.job AS a')
    ->join($schema.'.application_status AS b', 'b.id', 'a.app_status_id')
    ->whereRaw('EXISTS (
    SELECT
        1
    ...
    if($AccessMaintenanceJob->fleet_has_limit){
        $totalMarketResearchApproval->where(
            [
                'a.workshop_id' => auth()->user()->detail->workshop_id
            ]
        );
        $totalProcurementApproval->where(
            [
                'a.workshop_id' => auth()->user()->detail->workshop_id
            ]
        );
        $totalProcurementEvaluation->where(
            [
                'a.workshop_id' => auth()->user()->detail->workshop_id
            ]
        );
    ...
    $totalMarketResearchApproval = $totalMarketResearchApproval->count();
    $totalProcurementApproval = $totalProcurementApproval->count();
    $totalProcurementEvaluation = $totalProcurementEvaluation->count();



}

$jobDAO = new MaintenanceJobDAO();

use Illuminate\Http\Request;
$currentReq = new Request();
$myjob_total = $jobDAO->list($currentReq->merge(
    [
        'status_code' => 'my_job',
        'mode' => 'count'
    ]));
...
<head
    data-total-app="{{$totalAppointment}}"
    data-total-app1="{{$totalAssessment}}"
    data-total-app2="{{$totalVerification}}"
    data-total-app3="{{$totalApproval}}"
    data-total-app4="{{$totalCompleted}}"
    data-total-app5="{{$totalRejected}}"
    >
    ...
    <div class="sect-top">
        ...
        <div class="dropdown inline" style="display: inline;">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                ...
                <li><a class="dropdown-item {{$status_code == 'my_job' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('maintenance.job.list', ['status_code' =>  'my_job'])}}')">Tindakan Saya
                    @if($myjob_total>0)
                    <span class="badge bg-secondary">{{$myjob_total}}</span>
                    @endif
                </a></li>
                ...
                @if(auth()->user()->isEngineer() || auth()->user()->isEngineerMaintenance())
                        <li><a class="dropdown-item {{$status_code == '12' ? 'active':''}}" onclick="parent.openPgInFrame('{{route('maintenance.job.list', ['status_code' =>  '12'])}}')">Semakan Jadual Harga
                            @if($totalProcurementEvaluation>0)
                            <span class="badge bg-secondary">{{$totalProcurementEvaluation}}</span>
                            @endif
                        </a></li>
                    @endif
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group dropend">
                                        <button type="button" class="btn cux-btn" onClick="openPgInFrame('{{route('maintenance.job.register', [ 'id' =>  $maintenance->id, 'tab' => $tab, 'status_code' => $status_code])}}')"><i class="fa fa-pencil-alt"></i></button>
                                    </div>
                                </td>
                                <td><a href="{{route('maintenance.job.register', [ 'id' =>  $maintenance->id, 'tab' => $tab, 'status_code' => $status_code ])}}">{{$maintenance->ref_number}}</a></td>
                                <td><a href="{{route('maintenance.job.register', [ 'id' =>  $maintenance->id, 'tab' => $tab, 'status_code' => $status_code ])}}">{{$maintenance->applicant_name}}</a></td>
                                ...
                                <td>
                                    ...
                                    @if($maintenance && $maintenance->hasVehicle->count()>0)
                                        @foreach ($maintenance->hasVehicle as $vehicle)
                                            <label for="" style="font-weight: bold; color:black">{{$vehicle->plate_no}}</label><br/>
                                                @if($vehicle->hasExamform)
                                                    {{-- <div class=''> --}}
                                                        {{-- {{$vehicle->hasExamform->hasMaintenanceJobVehicleStatus->code}} --}}
                                                        {{-- {{$vehicle->hasExamform->hasExternalRepairStatus->code}} --}}
                                                        {{-- <small style='font-size:14px !important;'>{{$vehicle->hasExamform->hasMaintenanceJobVehicleStatus->desc}}</small> --}}
                                                        {{-- <small style='font-size:14px !important;'>{{ucwords(strtolower(trans($vehicle->hasExamform->hasMaintenanceJobVehicleStatus->desc)))}}</small>
                                                        @if($vehicle->hasExamform->hasInternalRepairStatus)
                                                            <br/>- <small style='font-size:14px !important;'>{{ucwords(strtolower(trans($vehicle->hasExamform->hasInternalRepairStatus->desc)))}}</small>
                                                        @endif
                                                        @if($vehicle->hasExamform->hasExternalRepairStatus)
                                                            <br/>- <small style='font-size:14px !important;'>{{ucwords(strtolower(trans($vehicle->hasExamform->hasExternalRepairStatus->desc)))}}</small>
                                                        @endif
                                                    </div> --}}
