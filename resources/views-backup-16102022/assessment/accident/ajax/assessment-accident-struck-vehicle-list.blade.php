            ...
            @foreach ($vehicleList as $index => $vehicle)
                <tr class="vehicle">
                    @if($hasAssessmentAccidentDetail->hasStatus->code == '01')
                        ...
                        <td class="lcal-btn" data-type="struck" data-current-page = "{{Request('page') ? Request('page') : 1}}" onclick="editAssessmentVehicle({{$vehicle->id}}, this)"><i class="fa fa-pencil-alt"></i></td>
                    @endif
                    <td>{{$vehicleList->firstItem() + $index}}</td>
                    <td class="special"><a data-type="struck" data-current-page = "{{Request('page') ? Request('page') : 1}}" onclick="editAssessmentVehicle({{$vehicle->id}}, this)" href="#">{{$vehicle->accwit_regno}}</a></td>
...
{{$vehicleList->links('pagination.ajax-default', [
   'targetDivList' =>  '#assessment_accident_struck_vehicle',
   'params' => $params
])}}
