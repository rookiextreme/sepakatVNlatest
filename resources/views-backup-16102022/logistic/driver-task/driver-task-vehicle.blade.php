...
$LogisticTaskDriverDAO->readVehicle($vehicle_id, $category);
$vehicle = $LogisticTaskDriverDAO->vehicle;
$detail = $LogisticTaskDriverDAO->vehicle->hasBooking;
...
<body class="content">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">
                <a href="{{ route('logistic.driver-task-vehicle') }}">Rekod Tugasan</a>
            </li>
        ...
        <div class="quick-navigation" data-fixed-after-touch="">
            <div class="wrapper" style="position: relative">
                <ul id="tabActive">
                    ...
                    <li class="cub-tab"  onClick="goTab(this, 'update_task');" id="tab2">{{$vehicle->hasStatus->code == '04' ? 'Tugasan' : 'Kemaskini Tugasan'}} </li>
                </ul>
                <div class="under-active d-none d-sm-block d-md-block">&nbsp;</div>
            </div>
        </div>
        <section id="detail" class="tab-content">
            @include('logistic.driver-task.tab.driver-task-vehicle-detail')
        </section>
        <section id="update_task" class="tab-content">
            @include('logistic.driver-task.tab.driver-update-task-vehicle')
        </section>
    ...
    <script type="text/javascript">
        ...
        function backToList(){
            window.location = "{{route('logistic.driver-task-vehicle')}}";
        }
        ...
        $(document).ready(function() {
            ...
            $('[xaction="task_done"]').on('click', function(e){
                ...
                $.post('{{route('logistic.driver.task.vehicle.done')}}', {
                    'ids': ids,
                    'category': '{{request('category')}}',
                    '_token':'{{ csrf_token() }}'
                }, function($data){
