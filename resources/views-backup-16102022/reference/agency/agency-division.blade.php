    ...
    <table class="table mt-0" id="mytable">
        <thead>
            <tr>
                <th style="width: 80px;">
                    <div class="btn-group">
                        <button class="btn cux-btn small" onclick="modalActionDivision(this, 'insert')"
        ...
        <tbody>
            ...
            @foreach ($divisions as $division)
                <tr id="">
                    <td style="width: 80px;">
                        <div class="btn-group">
                            <span class="btn cux-btn small" onclick="modalActionDivision(this, 'update')"
                            data-id="{{ $division->id }}" data-code="{{ $division->code }}"
                            data-name="{{ $division->desc }}" data-bs-toggle="modal"
                            data-bs-target="#divisionModal"><i
                                    class="fal fa-edit"></i></span>
                            <span onclick="deleteItemDivision({{$division->id}})" class="btn cux-btn small" xaction="delete_selected_division" data-bs-toggle="modal"
                            data-bs-target="#deleteModal" data-bs-target="" disabled type="button" data-toggle="popover"
                            data-trigger="focus" data-placement="top" data-content="Some info"><i
                                class="fal fa-trash-alt"></i></span>
                        </div>
                    </td>
                    <td style="width: 60px">{{ $division->code }}</td>
                    <td class="cfoc wd-wf key"><a href="#" onclick="modalActionDivision(this, 'update')"
                            data-id="{{ $division->id }}" data-code="{{ $division->code }}"
                            data-name="{{ $division->desc }}" data-bs-toggle="modal"
                            data-bs-target="#divisionModal">{{ $division->desc }}</a></td>
                    <td style="width: 30px" class="text-end" onclick="displayUnit(this, {{ $division->branch_id }}, {{ $division->id }},'{{ $division->code }}', '{{ $division->desc }}')">
                    <i id="angle" class="fa fa-angle-up fa-2x"></i></td>
                </tr>
                <tr xunitlist="division_id_{{$division->id}}_unit"></tr>
            @endforeach
...
<script type="text/javascript">
    ...
    function modalActionDivision(self, mode){
    ...
    function deleteItemDivision(id){
        division_ids = [id];
    }
    ...
    function getUnit(division_id){
        ...
        $.ajax({
            ...
            success: function (result) {
                console.log('[xunitlist="division_id_'+division_id+'_unit"]');
    ...
    function displayUnit(self, branch_id, division_id, unit_code, unit_name){
